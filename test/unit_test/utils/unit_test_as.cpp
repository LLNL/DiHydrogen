...
TEST_CASE("safe_as", "[utilities][as]")
{
  SECTION("Unsigned to signed integers")
  {
    ...
    CHECK_THROWS(h2::safe_as<int32_t>(std::numeric_limits<uint32_t>::max()));
  }
  ...
  SECTION("Big signed integers")
  {
    ...
    CHECK_NOTHROW(h2::safe_as<int64_t>(std::numeric_limits<int32_t>::max()));
  }

  SECTION("Big unsigned integers")
  {
    CHECK(h2::safe_as<uint32_t>(uint64_t{1}) == uint32_t{1});
    CHECK_THROWS(h2::safe_as<uint32_t>(std::numeric_limits<uint64_t>::max()));
    CHECK_NOTHROW(h2::safe_as<uint64_t>(std::numeric_limits<uint32_t>::max()));
  }
