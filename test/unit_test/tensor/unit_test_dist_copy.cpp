        ...
        [&](ShapeTuple grid_shape) {
                                        ...
                                        Distribution::Single})
          {
                                          ...
                                          Distribution::Single})
            {
              ...
              SrcTensorType src_tensor = SrcTensorType(SrcDev,
                                                       src_tensor_shape,
                                                       tensor_dim_types,
                                                       grid,
                                                       src_tensor_dist);
              DstTensorType dst_tensor = DstTensorType(DstDev,
                                                       dst_tensor_shape,
                                                       tensor_dim_types,
                                                       grid,
                                                       dst_tensor_dist);
              ...
              REQUIRE(dst_tensor.numel() == src_tensor.numel());
              REQUIRE_FALSE(dst_tensor.is_empty());
              REQUIRE(dst_tensor.is_local_empty()
                      == src_tensor.is_local_empty());
              REQUIRE_FALSE(dst_tensor.is_view());
              if (src_tensor.is_local_empty())
              {
                REQUIRE(dst_tensor.data() == nullptr);
              ...
              for (DataIndexType i = 0; i < src_tensor.local_numel(); ++i)
              {
                REQUIRE(read_ele<SrcDev>(
                          src_tensor.data(), i, src_tensor.get_stream())
                        == src_val);
                REQUIRE(read_ele<DstDev>(
                          dst_tensor.data(), i, dst_tensor.get_stream())
                        == src_val);
              }
            }
          }
