    ...
    for_comms([&](Comm& comm) {
      for_grid_shapes(
        [&](ShapeTuple grid_shape) {
          ...
          DistTTuple tensor_dist(
            TuplePad<DistTTuple>(grid.ndim(), Distribution::Block));
          ...
          for (typename ShapeTuple::size_type i = 0; i < local_shape.size();
               ++i)
          {
    ...
    for_comms([&](Comm& comm) {
      for_grid_shapes(
        [&](ShapeTuple grid_shape) {
          ...
          DTTuple tensor_dim_types(TuplePad<DTTuple>(grid.ndim(), DT::Any));
          DistTTuple tensor_dist(
            TuplePad<DistTTuple>(grid.ndim(), Distribution::Replicated));
          DistTensorType tensor = DistTensorType(
            Dev, tensor_shape, tensor_dim_types, grid, tensor_dist);
    ...
    for_comms([&](Comm& comm) {
      for_grid_shapes(
        [&](ShapeTuple grid_shape) {
          ...
          DTTuple tensor_dim_types(TuplePad<DTTuple>(grid.ndim(), DT::Any));
          DistTTuple tensor_dist(
            TuplePad<DistTTuple>(grid.ndim(), Distribution::Single));
          DistTensorType tensor = DistTensorType(
            Dev, tensor_shape, tensor_dim_types, grid, tensor_dist);
          ...
          REQUIRE(tensor.proc_grid() == grid);
          REQUIRE(tensor.distribution() == tensor_dist);
          for (typename ShapeTuple::size_type i = 0; i < tensor_shape.size();
               ++i)
          {
            REQUIRE(tensor.shape(i) == tensor_shape[i]);
            REQUIRE(tensor.dim_type(i) == tensor_dim_types[i]);
                        ...
                        AllDevList)
{
  ...
  REQUIRE_FALSE(DistTensorType(
                  Dev, {4}, {DT::Any}, grid, {Distribution::Block}, StrictAlloc)
                  .is_lazy());
  REQUIRE(
    DistTensorType(Dev, {4}, {DT::Any}, grid, {Distribution::Block}, LazyAlloc)
      .is_lazy());
}
    ...
    for_comms([&](Comm& comm) {
      for_grid_shapes(
        [&](ShapeTuple grid_shape) {
          ...
          DataIndexType new_local_numel =
            product<DataIndexType>(new_local_shape);
      ...
      for_grid_shapes(
        [&](ShapeTuple grid_shape) {
                                    ...
                                    Distribution::Single})
          {
            ...
            std::unique_ptr<DistTensorType> view =
              tensor.view(IndexRangeTuple{});
            REQUIRE(view->shape() == ShapeTuple{});
            REQUIRE(view->local_shape() == ShapeTuple{});
            REQUIRE(view->dim_types() == DTTuple{});
            REQUIRE(view->distribution() == DistTTuple{});
