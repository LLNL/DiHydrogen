...
TEST_CASE("Switch dispatcher", "[utilities][multimethods]")
{
  ...
  {
    using Dispatcher = SwitchDispatcher<TestFunctor,
                                        int,
                                        base,
                                        DTypes,
                                        base,
    ...
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d1_b, *d1_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d1_b, *d2_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d1_b, *d3_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d2_b, *d1_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d2_b, *d2_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d2_b, *d3_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d3_b, *d1_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d3_b, *d2_b, *d3_b),
                    DeductionException);
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d3_b, *d3_b, *d3_b),
                    DeductionException);
  }
  ...
  {
    using Dispatcher =
      SwitchDispatcher<TestFunctorWithArgs, int, base, DTypes, base, DTypes>;
    ...
    // Dispatch error
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d2_b, *d3_b, 13), DispatchException);

    // Deduction error
    derived_four d4;
    base* d4_b = &d4;
    CHECK_THROWS_AS(Dispatcher::Exec(f, *d2_b, *d4_b, 13), DeductionException);
  }
}
