...
static_assert(h2::all_of(test_tuple, [](TestFixedSizeTuple::type x) {
  return x == 1 || x == 2;
}));
static_assert(!h2::all_of(test_tuple, [](TestFixedSizeTuple::type x) {
  return x == 1 || x == 3;
...
static_assert(h2::map(test_tuple,
              ...
              == h2::FixedSizeTuple<bool,
                                    ...
                                    TestFixedSizeTuple::max_size>(true, false));
static_assert(h2::map_index(test_tuple,
                            [](TestFixedSizeTuple::size_type x) {
...
constexpr TestFixedSizeTuple
  test_tuple_move_construct(std::move(test_tuple_copy));
