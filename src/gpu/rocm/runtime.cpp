...
static char const* rsmi_status_to_string(rsmi_status_t const status)
{
  ...
  case RSMI_STATUS_INTERNAL_EXCEPTION: return "RSMI_STATUS_INTERNAL_EXCEPTION";
  case RSMI_STATUS_INPUT_OUT_OF_BOUNDS:
...
static std::string get_device_name_by_pci_bus(int const pci_bus)
{
  ...
  if (dev_id == ndevices)
  {
    throw std::runtime_error("Could not find device with matching PCI bus ID");
  }
...
{
  int pci;
  H2_CHECK_HIP(hipDeviceGetAttribute(&pci, hipDeviceAttributePciBusId, gpu_id));
...
void h2::gpu::init_runtime()
{
  if (!initialized_)
  {
    ...
    H2_GPU_TRACE(is_integrated_ ? "GPU is integrated"
...
void h2::gpu::record_event(hipEvent_t const event, hipStream_t const stream)
{
  H2_GPU_TRACE(
    "recording event {} on stream {}", (void*) event, (void*) stream);
