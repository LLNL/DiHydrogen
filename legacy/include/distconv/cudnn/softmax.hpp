...
template <>
class Softmax<cudnn::BackendCUDNN> {
 public:
  Softmax(cudnn::BackendCUDNN &backend): m_be(backend) {}
 ...
 protected:
  ...
  void allreduce(DataType *sample_values, int num_samples,
                 bool max_or_sum) {
    if (m_num_procs_per_sample < 2) return;
