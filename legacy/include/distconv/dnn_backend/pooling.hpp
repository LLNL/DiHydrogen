...
public:
             ...
             std::string const& mode)
  {
    ...
    for (int i = 0; i < m_num_spatial_dims; ++i)
    {
      ...
      {
        assert_always(input.get_distribution().get_split_shape()[i] == 1);
      }
    }
    ...
    GPUDNNBackend::setup_tensor_descriptor(
      m_input_d, input, IntVector(m_halo_fwd_recv), IntVector(m_halo_bwd_recv));
...
private:
  ...
  using HaloExchangeMPI =
    tensor::HaloExchangeMPI<DataType, tensor::CUDAAllocator, Al::NCCLBackend>;
  using HaloExchangeAL =
    tensor::HaloExchangeAL<DataType, tensor::CUDAAllocator, Al::NCCLBackend>;
#ifdef DISTCONV_HAS_P2P
  using HaloExchangeP2P =
    tensor::HaloExchangeP2P<DataType, tensor::CUDAAllocator, Al::NCCLBackend>;
  ...
  using HaloExchangeNVSHMEMGraph = tensor::
    HaloExchangeNVSHMEMGraph<DataType, tensor::CUDAAllocator, Al::NCCLBackend>;
#endif  // DISTCONV_HAS_CUDA_GRAPH
  using HaloExchangeNVSHMEMDirect = tensor::
    HaloExchangeNVSHMEMDirect<DataType, tensor::CUDAAllocator, Al::NCCLBackend>;
  ...
  void bp_accumulate_sum(
    tensor::Tensor<DataType, tensor::LocaleMPI, tensor::CUDAAllocator>& tensor,
  ...
  template <typename Allocator>
  void
  exchange_halo_reverse(tensor::Tensor<DataType, LocaleMPI, Allocator>& tensor,
                        std::unique_ptr<HaloExchange>& xch)
  {
    if (m_be.profiling())
