...
public:
      ...
      m_global_stats(global_stats)
  {
    ...
    {
      m_allreducer = std::make_unique<tensor::AllreduceNVSHMEM<DataType>>(
        m_stream, tensor::AllreduceNVSHMEM<DataType>::RECURSIVE_DOUBLING_HOST);
    }
    else if (m_impl == BatchnormImpl::NVSHMEM_RECURSIVE_DOUBLING)
    {
      m_allreducer = std::make_unique<tensor::AllreduceNVSHMEM<DataType>>(
        m_stream, tensor::AllreduceNVSHMEM<DataType>::RECURSIVE_DOUBLING);
    }
               ...
               Tensor& d_input)
  {
    ...
    backward_stage2(
      input, d_output, mean, var, scale, mean_gradient, var_gradient, d_input);
    return 0;
  }
...
private:
  ...
  void sums_to_statistics(index_t num_per_sum,
                          ...
                          Tensor& running_var)
  {
    batchnorm::sums_to_statistics<Tensor>(
      num_per_sum, m_decay, mean, var, running_mean, running_var, m_stream);
  }
