...
public:
      ...
      m_d_input_all_channels_d{GPUDNNBackend::make_tensor_descriptor()}
  {
    apply_to_spatial_sides(m_num_dims, [this](int i, Side side) {
      m_input_boundaries_d(i, side) = GPUDNNBackend::make_tensor_descriptor();
      m_output_boundaries_d(i, side) = GPUDNNBackend::make_tensor_descriptor();
    });
             ...
             bool deconv = false)
  {
    ...
    setup_tensor_descriptors(
      input, filter, output, d_input, d_filter, d_output, strides, dilations);
  ...
  void setup_bias(tensor::Tensor<DataType, LocaleMPI, Allocator> const& bias)
  {
    ...
    IntVector strides = tensor::get_strides(shape, bias.get_halo_width(), 1);
    GPUDNNBackend::set_tensor_descriptor(m_bias_d,
                                         dt,
                                         shape.num_dims(),
    ...
    tensor::Tensor<DataType, LocaleMPI, Allocator> const& d_bias)
  {
    ...
    IntVector strides = tensor::get_strides(shape, d_bias.get_halo_width(), 1);
    GPUDNNBackend::set_tensor_descriptor(m_d_bias_d,
                                         dt,
                                         shape.num_dims(),
                                         util::reverse(IntVector(shape)).data(),
                                         util::reverse(strides).data());
              ...
              bool inference = false)
  {
    ...
    if (!m_overlap_halo_exchange_fwd)
    {
      ...
      if (m_chanfilt_algo == ChannelParallelismAlgorithm::X)
      {
        ensure_tensors_conform(
          input, m_output_all_filters_t, filter, "stationary-x forward");
      ...
      else if (m_chanfilt_algo == ChannelParallelismAlgorithm::Y)
      {
        ...
        ensure_tensor_descriptors_conform(
          m_input_gathered_d, m_output_d, m_filter_d, "stationary-y forward");
        m_be.convolution_forward(alpha,
                                 m_input_gathered_d,
    ...
    else
    {
      ...
      apply_to_spatial_sides(m_num_dims, [&](int i, Side side) {
        ...
        void* ws_boundary = internal::RuntimeGPU::get_device_memory_pool().get(
        ...
        internal::RuntimeGPU::get_device_memory_pool().release(ws_boundary);
    ...
    bool dump_profile = false)
  {
    ...
    void* d_input_ptr =
      d_input.get_base_ptr() - d_input.get_local_offset(m_halo_bwd_recv, true);
    ...
    {
      ensure_tensors_conform(
        d_input, m_d_output_gathered_t, filter, "stationary-x backward-data");
    ...
    else
    {
      if (!m_deconv)
      {
        ensure_tensors_conform(d_input, d_output, filter, "backward-data");
        ensure_tensor_descriptors_conform(
    ...
    bool dump_profile = false)
  {
    ...
    else
    {
      ...
      else
      {
        if (!m_deconv)
        {
          ensure_tensors_conform(input, d_output, d_filter, "backward-filter");
          ensure_tensor_descriptors_conform(
  ...
  void set_num_samples(int n)
  {
    ...
    if (n != GPUDNNBackend::get_tensor_num_samples(m_input_d))
    {
      ...
      if (m_chanfilt_algo == ChannelParallelismAlgorithm::X
          || m_chanfilt_algo == ChannelParallelismAlgorithm::W)
      {
        GPUDNNBackend::set_tensor_num_samples(m_output_all_filters_d, n);
          ...
          || m_chanfilt_algo == ChannelParallelismAlgorithm::W)
      {
        ...
        GPUDNNBackend::set_tensor_num_samples(m_d_input_all_channels_d, n);
      }
    }
  }
...
protected:
  ...
  BoundaryAttributesV<GPUDNNBackend::TensorDescriptor_t> m_output_boundaries_d;
  ...
  void record_start_exchange()
  {
    if (m_enable_profiling)
    {
      GPUDNNBackend::record_event(m_event_exchange_start, m_be.get_stream());
    }
  }
  ...
  void record_end_exchange()
  {
    if (m_enable_profiling)
    {
      GPUDNNBackend::record_event(m_event_exchange_end, m_be.get_stream());
    }
  }
                               ...
                               bool is_forward)
  {
    ...
    if (has_boundary)
    {
      ...
      {
        apply_to_spatial_sides(m_num_dims, [&ss, this](int i, Side side) {
          if (!m_boundary_req(i, side))
            return;
          float const elapsed = GPUDNNBackend::elapsed_time(
            m_event_start_boundaries(i, side), m_event_end_boundaries(i, side));
    ...
    int_vector const& dilations)
  {
    ...
    if (m_overlap_halo_exchange_fwd)
    {
      util::MPIPrintStreamDebug() << "input interior: " << m_input_interior_d;
      apply_to_spatial_sides(m_num_dims, [this](int i, Side side) {
        if (m_boundary_req(i, side))
        {
          util::MPIPrintStreamDebug()
    ...
    if (m_overlap_halo_exchange_fwd)
    {
      util::MPIPrintStreamDebug() << "output interior: " << m_output_interior_d;
      apply_to_spatial_sides(m_num_dims, [this](int i, Side side) {
        if (m_boundary_req(i, side))
        {
          util::MPIPrintStreamDebug()
    ...
    int_vector const& dilations)
  {
    ...
    apply_to_spatial_sides(m_num_dims, [&](int dim, Side side) {
      ...
      util::MPIPrintStreamDebug()
        << "Side: " << side << ", dim: " << dim << "; " << num_boundary_centers
        << ", " << h << ", " << interior_offset;
      ...
      GPUDNNBackend::copy_tensor_descriptor(m_input_boundaries_d(dim, side),
                                            m_input_d);
      GPUDNNBackend::set_tensor_dimension(
        m_input_boundaries_d(dim, side), dim, input_boundary_dim);
      GPUDNNBackend::copy_tensor_descriptor(m_output_boundaries_d(dim, side),
                                            m_output_d);
      GPUDNNBackend::set_tensor_dimension(
        m_output_boundaries_d(dim, side), dim, output_boundary_dim);
      setup_boundary_offsets(
        dim, side, input, output, input_boundary_dim, output_boundary_dim);
    });
    ...
    {
      {
        ...
        release_tmp_tensor_buffer(m_output_all_filters_t);
        util::MPIPrintStreamDebug()
          << "output_all_filters descriptor: " << m_output_all_filters_d;
      }
      {
        auto dist = tensor::Distribution::make_shared_distribution(
        ...
        release_tmp_tensor_buffer(m_d_output_gathered_t);
        util::MPIPrintStreamDebug()
          << "d_output_gathered descriptor: " << m_d_output_gathered_d;
      }
    }
    else if (m_chanfilt_algo == ChannelParallelismAlgorithm::Y)
    {
      ...
      }
      {
        ...
        util::MPIPrintStreamDebug()
          << "d_input_all_channels tensor: " << m_d_input_all_channels_t;
        get_tmp_tensor_buffer(m_d_input_all_channels_t);
        GPUDNNBackend::setup_tensor_descriptor(
          m_d_input_all_channels_d,
          m_d_input_all_channels_t,
        ...
        release_tmp_tensor_buffer(m_d_input_all_channels_t);
        util::MPIPrintStreamDebug()
          << "d_input_all_channels descriptor: " << m_d_input_all_channels_d;
      }
    }
    else if (m_chanfilt_algo == ChannelParallelismAlgorithm::W)
    {
    ...
    tensor::Tensor<DataType, LocaleMPI, Allocator> const& filter)
  {
    ...
    if (m_be.get_segmented_ar_comm(m_chanfilt_segments) == nullptr)
    {
      m_be.init_segmented_ar_comm(
        m_chanfilt_segments, input.get_sub_locale_except_dim(-2).get_comm());
    }
    if (m_chanfilt_algo == ChannelParallelismAlgorithm::X
        || m_chanfilt_algo == ChannelParallelismAlgorithm::Y)
    {
                            ...
                            size_t ws_size = 0)
  {
    ...
    if (!m_overlap_halo_exchange_fwd)
    {
      ...
      else
      {
        ...
        {
          ensure_tensor_descriptors_conform(
            m_input_d, m_output_d, m_filter_d, "setup algos forward");
          m_fwd_algo = m_be.get_fwd_algorithm(m_fwd_find_algo,
                                              m_input_d,
                                              input,
                                              m_filter_d,
      ...
      util::MPIPrintStreamDebug() << "Convolution forward algorithm: "
      ...
      apply_to_spatial_sides(m_num_dims, [&](int i, Side side) {
        if (m_boundary_req(i, side))
        {
          ...
          util::MPIPrintStreamDebug()
            ...
            << side << "): " << util::get_name(m_fwd_boundary_algos(i, side));
        }
      });
    }
                                 ...
                                 size_t ws_size = 0)
  {
    ...
    else
    {
      if (!m_skip_bp_data)
      {
        ...
        {
          ensure_tensor_descriptors_conform(
            m_d_input_d, m_d_output_d, m_filter_d, "setup algos backward-data");
                                   ...
                                   size_t ws_size = 0)
  {
    ...
    else
    {
      ...
      {
        ensure_tensor_descriptors_conform(
          m_input_d, m_d_output_d, m_filter_d, "setup algos backward-filter");
        m_bwd_filter_algo =
          m_be.get_bwd_filter_algorithm(m_bwd_filter_find_algo,
                                        m_input_d,
                                        input,
    ...
    if (!m_overlap_halo_exchange_fwd)
    {
      ...
      else if (m_chanfilt_algo == ChannelParallelismAlgorithm::Y)
      {
        ...
        s = m_be.get_conv_forward_workspace_size(
          m_input_gathered_d, m_filter_d, m_conv_fwd_d, m_output_d, m_fwd_algo);
      }
      else if (m_chanfilt_algo == ChannelParallelismAlgorithm::W)
      {
        ensure_tensor_descriptors_conform(m_input_gathered_d,
  ...
  void setup_workspace_size_fwd_boundaries()
  {
    ...
    apply_to_spatial_sides(m_num_dims, [this](int i, Side side) {
      ...
      {
        size_t const s =
          m_be.get_conv_forward_workspace_size(m_input_boundaries_d(i, side),
                                               m_filter_d,
                                               m_conv_fwd_d,
                                               m_output_boundaries_d(i, side),
                                               m_fwd_boundary_algos(i, side));
  ...
  void setup_workspace_size_bwd_data()
  {
    ...
    {
      ensure_tensor_descriptors_conform(
        m_d_input_d, m_d_output_d, m_filter_d, "workspace backward-data");
      s = get_workspace_size_bwd_data(m_filter_d, m_d_output_d, m_d_input_d);
    }
    m_ws_size_bwd_data = s;
  }

  size_t get_workspace_size_bwd_data(GPUDNNBackend::FilterDescriptor_t filter,
  ...
  void setup_workspace_size_bwd_filter()
  {
    ...
    {
      ensure_tensor_descriptors_conform(
        m_input_d, m_d_output_d, m_d_filter_d, "workspace backward-filter");
                                ...
                                GPUDNNBackend::FilterDescriptor_t d_filter)
  {
    ...
    if (!m_deconv)
    {
      s = m_be.get_conv_bwd_filter_workspace_size(
        input, d_output, m_conv_bwd_filter_d, d_filter, m_bwd_filter_algo);
    }
    else
    {
      s = m_be.get_conv_bwd_filter_workspace_size(
        d_output, input, m_conv_bwd_filter_d, d_filter, m_bwd_filter_algo);
    }
    return s;
  }
    ...
    GPUDNNBackend::ConvolutionDescriptor_t& desc_bp_filter)
  {
    ...
    {
      auto df =
        internal::get_dilated_filter_size((int) filter_shape[i], dilations[i]);
      if (!(pads[i] * 2 + 1 == df || pads[i] == 0))
      {
        util::MPIPrintStreamError()
          << "Padding size must be zero or must match the filter "
             "size: "
          << "padding_w: " << pads[i] << ", filter shape: " << filter_shape;
        std::abort();
      }
    }
    ...
    {
      auto window_dim =
        internal::get_dilated_filter_size<int>(stencil_dims[i], dilations[i]);
      if (window_dim % 2)
      {
        stencil_dims[i] = (window_dim - 1) / 2;
      }
    ...
    if (!m_skip_bp_data)
    {
      GPUDNNBackend::set_convolution_descriptor(desc_bp_data,
                                                ...
                                                mode,
                                                dt);
      GPUDNNBackend::set_convolution_group_count(desc_bp_data, num_groups);
    }
                     ...
                     bool skip_unpack = false)
  {
    ...
    xch.exchange(
      m_boundary_comms, m_be.get_stream(), false, !async, false, skip_unpack);
    if (m_be.profiling())
    {
      GPU_PROFILE_RANGE_POP();
    }
  ...
  void setup_boundary_streams(IndexVector const& split_idx)
  {
    apply_to_spatial_sides(m_num_dims, [this](int i, Side side) {
      ...
      m_boundary_streams(i, side) = m_be.get_internal_priority_stream(idx);
  ...
  template <typename Allocator>
  void
  allreduce_gradients(tensor::Tensor<DataType, LocaleMPI, Allocator>& gradients)
  {
    if (m_chanfilt_algo == ChannelParallelismAlgorithm::NONE)
    {
  ...
  template <typename Allocator>
  void get_tmp_tensor_buffer(tensor::Tensor<DataType, LocaleMPI, Allocator>& t)
  {
    DataType* buf =
    ...
    std::string const& context)
  {
    ...
    {
      util::MPIPrintStreamError() << "Unsupported num_dims " << m_num_dims;
      std::abort();
    }
  }
  ...
  {
    int num_samples = GPUDNNBackend::get_tensor_num_samples(m_input_d);
    cache[num_samples] = AlgoTuple(
      m_fwd_algo, m_bwd_data_algo, m_bwd_filter_algo, m_fwd_boundary_algos);
  }
