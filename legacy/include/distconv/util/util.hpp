...
public:
    PrintStreamDebug(const std::string& prefix = "[DEBUG] ")
        : PrintStream(
            get_env_bool("DISTCONV_PRINT_DEBUG", true), std::cerr, prefix)
    {}
};
#else
...
public:
    PrintStreamWarning()
        : PrintStream(get_env_bool("DISTCONV_PRINT_WARNING", true),
                      std::cerr,
                      "[WARNING] ")
    {}
};
