...
#define DISTCONV_CHECK_CUDNN(cudnn_call)                                       \
  ...
  {                                                                            \
    ...
    {                                                                          \
      std::stringstream ss;                                                    \
      ss << "cuDNN error: " << cudnnGetErrorString(cudnn_status) << std::endl  \
...
struct CUDNNConvolutionFwdAlgorithms
{
  ...
  algo_pair algos[num] = {
    ...
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_GEMM, "GEMM"),
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_DIRECT, "DIRECT"),
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_FFT, "FFT"),
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_FFT_TILING, "FFT_TILING"),
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD, "WINOGRAD"),
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD_NONFUSED,
                   "WINOGRAD_NONFUSED"),
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM, "DEFAULT"),
    std::make_pair(CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM, "DETERMINISTIC")};
...
struct CUDNNConvolutionBwdDataAlgorithms
{
  ...
  algo_pair algos[num] = {
    ...
    std::make_pair(CUDNN_CONVOLUTION_BWD_DATA_ALGO_1, "ALGO_1"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_DATA_ALGO_FFT, "FFT"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_DATA_ALGO_FFT_TILING, "FFT_TILING"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_DATA_ALGO_WINOGRAD, "WINOGRAD"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_DATA_ALGO_WINOGRAD_NONFUSED,
                   "WINOGRAD_NONFUSED"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_DATA_ALGO_0, "DEFAULT"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_DATA_ALGO_1, "DETERMINISTIC")};
...
struct CUDNNConvolutionBwdFilterAlgorithms
{
  ...
  algo_pair algos[num] = {
    ...
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_1, "ALGO_1"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_3, "ALGO_3"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_FFT, "FFT"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_FFT_TILING, "FFT_TILING"),
    // This is not listed in the API document, but available in the
    // header file, and indeed can be chosen by the search method
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_WINOGRAD, "WINOGRAD"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_WINOGRAD_NONFUSED,
                   "WINOGRAD_NONFUSED"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_0, "DEFAULT"),
    std::make_pair(CUDNN_CONVOLUTION_BWD_FILTER_ALGO_1, "DETERMINISTIC")};
...
inline std::string tostring(cudnnFilterDescriptor_t const& desc)
{
  ...
  DISTCONV_CHECK_CUDNN(
    cudnnGetFilterNdDescriptor(desc, req_num_dim, &dt, &fmt, &num_dims, dims));
  std::stringstream ss;
...
inline std::string tostring(cudnnConvolutionDescriptor_t const& desc)
{
  ...
  cudnnDataType_t dt;
  DISTCONV_CHECK_CUDNN(cudnnGetConvolutionNdDescriptor(desc,
                                                       req_array_length,
                                                       &array_length,
                                                       pads,
                                                       strides,
