...
struct PairwiseSyncDevice
{
  ...
  PairwiseSyncDevice(CounterType* local_counter, CounterType* shmem_counter)
  ...
  __device__ __forceinline__ void inc_counter() { ++(*m_local_counter); }

  __device__ __forceinline__ void
  sync(int peer, bool do_notify, bool do_wait, SyncType sync_type)
  {
    if (do_notify)
    {
...
struct SyncArrayDevice
{
  ...
  SyncArrayDevice(CounterType* local_counter, CounterType* shmem_counter)
    : m_local_counter(local_counter), m_shmem_counter(shmem_counter)
  {}
...
#else
    nvshmem_long_put_nbi(
      (CounterType*) (m_shmem_counter + idx), m_local_counter + idx, 1, peer);
#endif
  }
  ...
  __device__ __forceinline__ void
  sync(int peer, bool do_notify, bool do_wait, SyncType sync_type, int idx)
  {
    if (do_notify)
    {
...
public:
  SyncArray(size_t size)
    : m_local_counter(nullptr), m_shmem_counter(nullptr), m_size(size)
  {}
  void alloc_counters();
  void init_counters();
...
#define DEFINE_PUT(TYPE)                                                       \
  ...
  {                                                                            \
    nvshmem_##TYPE##_put_nbi(                                                  \
      (TYPE*) dest, (const TYPE*) source, nelems * 2, pe);                     \
  }                                                                            \
  inline __device__ void put_block(                                            \
    TYPE* dest, const TYPE* source, size_t nelems, int pe)                     \
  {                                                                            \
  ...
  inline __device__ void put_block(                                            \
    TYPE##2 * dest, const TYPE##2 * source, size_t nelems, int pe)             \
  {                                                                            \
    nvshmemx_##TYPE##_put_block(                                               \
      (TYPE*) dest, (const TYPE*) source, nelems * 2, pe);                     \
  }                                                                            \
  inline __device__ void put_nbi_block(                                        \
    TYPE* dest, const TYPE* source, size_t nelems, int pe)                     \
  {                                                                            \
...
#define DEFINE_SUM_TO_ALL(TYPE)                                                \
  inline void sum_to_all_on_stream(TYPE* target,                               \
                                   const TYPE* source,                         \
                                   int nreduce,                                \
                                   int PE_start,                               \
                                   int logPE_stride,                           \
                                   ...
                                   cudaStream_t s)                             \
  {                                                                            \
    nvshmemx_##TYPE##_sum_reduce_on_stream(                                    \
      NVSHMEM_TEAM_WORLD, target, source, nreduce, s);                         \
  }
