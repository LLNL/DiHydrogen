...
void wait_stream(cudaStream_t master,
                 cudaStream_t* followers,
                 int num_followers);
...
struct Clock
{
  ...
  void start() { DISTCONV_CHECK_CUDA(cudaEventRecord(m_ev1, m_s)); }
  void stop() { DISTCONV_CHECK_CUDA(cudaEventRecord(m_ev2, m_s)); }
  float get_time()
  {
