...
public:
    ...
    for (int pid = 0; pid < num_ranks; ++pid)
    {
      if (m_send_counts[pid] != 0 || m_recv_counts[pid] != 0)
      {
        util::MPIPrintStreamDebug()
          << "Send/recv counts for " << pid << ": " << m_send_counts[pid]
          << ", " << m_recv_counts[pid];
    ...
    for (int i = 0; i < num_dims; ++i)
    {
      ...
      for (int j = 0; j < dst_locale_dim; ++j)
      {
        ...
        for (int next = j + 1; next < dst_locale_dim; ++next)
        {
          ...
          {
            next_rank_global_offset =
              dst_tensor.get_dimension_rank_offset(i, next);
            break;
          }
        }
        index_t lim;
  ...
  void setup_displs(TensorType const& src_tensor, TensorType const& dst_tensor)
  {
    ...
    util::MPIPrintStreamDebug() << "src_local_region: " << src_local_region
    ...
    for (int pid = 0; pid < num_ranks; ++pid)
    {
      ...
      {
        Region dst_remote_region(dst_tensor.get_remote_index(dst_pid_idx),
        ...
        util::MPIPrintStreamDebug()
          << "send_intersection for " << pid << ": " << send_intersection
      ...
      else
      {
        ...
        util::MPIPrintStreamDebug()
          << "destination " << pid << " is not a split root";
        m_send_counts[pid] = 0;
      }
      ...
      {
        Region src_remote_region(src_tensor.get_remote_index(src_pid_idx),
  ...
  int get_num_dims() const { return m_src_locale_shape.num_dims(); }

  template <typename BufAlloc, typename BufDel>
  std::shared_ptr<DataType>
  get_src_buf(bool is_forward, StreamType stream, BufAlloc alloc, BufDel del)
  {
    ...
    else
    {
      ...
      DataType* buf = buffer_count == 0 ? nullptr : alloc(buffer_count, stream);
  ...
  template <typename BufAlloc, typename BufDel>
  std::shared_ptr<DataType>
  get_dst_buf(bool is_forward, StreamType stream, BufAlloc alloc, BufDel del)
  {
    auto non_delete = [](DataType* p) {
    };
...
protected:
  ...
  bool is_sample_to_spatial(TensorType const& src, TensorType const& dst)
  {
    ...
    {
      if (src.get_split_shape()[i] != 1)
        return false;
    }

    // No sharing on the sample dimension
    if (src.get_split_shape()[-1] != src.get_locale_shape()[-1])
      return false;
    if (dst.get_split_shape()[-1] != dst.get_locale_shape()[-1])
      return false;

    // Unpack assumes only receving from a single source, which means
    // the sample dimension must be equal size
    if (src.get_locale_shape()[-1] != dst.get_locale_shape()[-1])
      return false;
                        ...
                        size_t& dst_offset)
  {
    ...
    for (int i = 0; i < ND; ++i)
    {
      ...
      if (rank_limits[rank_limits_idx] == -1)
      {
        ...
        dst_buffer_offset =
          std::min(src_global_index % dst_local_dim, src_local_idx[i]);
        dst_buffer_dim = std::min(
          (int) (src_local_shape[i] - (src_local_idx[i] - dst_buffer_offset)),
          dst_local_dim);
      }
      else
      {
        ...
        int dst_rank_base = dst_rank_idx == 0
                              ? 0
      ...
      for (int p2 = 0; p2 < (int) dst_locale_shape[2]; ++p2)
      {
        for (int i2 = 0; i2 < (int) dst_local_shape[2]; ++i2)
        {
          for (int p1 = 0; p1 < (int) dst_locale_shape[1]; ++p1)
          {
            ...
            {
              index_t src_offset = CALC_OFFSET4(0,
                                                i1 + dst_local_shape[1] * p1,
                                                i2 + dst_local_shape[2] * p2,
                                                i3 + dst_local_shape[3] * p3,
                                                src_local_strides);
              index_t dst_offset_i12 =
                CALC_OFFSET4(0, i1, i2, i3, dst_local_strides);
              for (int p0 = 0; p0 < (int) dst_locale_shape[0]; ++p0)
              {
                int dst_rank_idx =
                  CALC_OFFSET4(p0, p1, p2, p3, dst_locale_strides);
            ...
            for (int i2 = 0; i2 < (int) dst_local_shape[2]; ++i2)
            {
              for (int p1 = 0; p1 < (int) dst_locale_shape[1]; ++p1)
              {
                for (int i1 = 0; i1 < (int) dst_local_shape[1]; ++i1)
                {
                  ...
                  {
                    index_t src_offset =
                      CALC_OFFSET5(0 + dst_local_shape[0] * p0,
                                   i1 + dst_local_shape[1] * p1,
                                   i2 + dst_local_shape[2] * p2,
                                   i3 + dst_local_shape[3] * p3,
...
#pragma omp parallel for collapse(3)
    for (int i3 = 0; i3 < (int) dst_local_shape[3]; ++i3)
    {
      for (int i2 = 0; i2 < (int) dst_local_shape[2]; ++i2)
      {
        ...
        {
          int i0 = 0;
          index_t dst_offset = CALC_OFFSET4(i0, i1, i2, i3, dst_strides);
          index_t buf_offset = CALC_OFFSET4(i0, i1, i2, i3, buf_strides);
          std::memcpy(
            &dst[dst_offset], &buf[buf_offset], sizeof(DataType) * linear_len);
        }
      }
    }
  }
    ...
    for (int i4 = 0; i4 < (int) dst_local_shape[4]; ++i4)
    {
      for (int i3 = 0; i3 < (int) dst_local_shape[3]; ++i3)
      {
        for (int i2 = 0; i2 < (int) dst_local_shape[2]; ++i2)
        {
          ...
          {
            constexpr int i0 = 0;
            index_t dst_offset = CALC_OFFSET5(i0, i1, i2, i3, i4, dst_strides);
            index_t buf_offset = CALC_OFFSET5(i0, i1, i2, i3, i4, buf_strides);
