...
class Memory {
 ...
 public:
  ...
  Memory():
      ...
      m_alias_ptr(nullptr), m_alias_const_ptr(nullptr) {}
  ...
  Memory<Allocator> &operator=(
      Memory<Allocator> m) {
    swap(*this, m);
    return *this;
  }

  friend void swap(Memory<Allocator> &x,
                   Memory<Allocator> &y) {
    using std::swap;
  ...
  size_t get_size() const {
    return m_property->m_size;
  }

  size_t get_ldim() const {
    return m_property->m_ldim;
  }

  size_t get_pitch() const {
    return m_property->m_pitch;
  }

  size_t get_real_size() const {
    return get_size() / get_ldim() * get_pitch();
  }

  bool is_pitched() const {
    return get_pitch() != get_ldim();
  }
  ...
  int memset(int v, typename Stream<Allocator>::type stream=
             Stream<Allocator>::default_value) {
    void *dst = get();
    if (dst == nullptr) return -1;
    Allocator::memset(dst, get_pitch(), v, get_size(), get_ldim(),
                      stream);
    return 0;
  }
  ...
  std::ostream &print(std::ostream &os) const {
    ...
    ss << "("
       << *m_property
       << ", managed ptr: " << m_managed_ptr.get()
       << ", alias ptr: " << m_alias_ptr
       << ", alias const ptr: " << m_alias_ptr
       << ")";
    os << ss.str();
    return os;
  }
...
inline typename std::enable_if<
  ...
  std::is_same<AllocSrc, BaseAllocator>::value, int>::type
Copy(Memory<AllocDst> &dst, const Memory<AllocSrc> &src,
  ...
  for (size_t y = 0; y < y_len; ++y) {
    size_t src_offset = x_src_offset +
        src.get_pitch() * (y_src_offset + y);
    size_t dst_offset = x_dst_offset +
        dst.get_pitch() * (y_dst_offset + y);
    std::memcpy((char*)dst.get() + dst_offset,
                (char*)src.get() + src_offset,
                x_len);
  }
  return 0;
}
