...
public:
  ...
  void dump_packed_halo(int dim)
  {
    ...
    for (auto side : SIDES)
    {
      ...
      h2::gpu::mem_copy((void*) h,
                        get_send_buffer(dim, side),
                        get_halo_size(dim) * sizeof(DataType));
      std::ostringstream file_path_send;
      file_path_send << "send_halo_" << rank << "_" << dim << "_" << side
                     << ".txt";
      ...
      file_path_recv << "recv_halo_" << rank << "_" << dim << "_" << side
                     << ".txt";
  ...
  void dump_recv_halo(int dim)
  {
    ...
    for (auto side : SIDES)
    {
      ...
      file_path << "recv_halo_" << rank << "_" << dim << "_" << side << ".txt";
...
protected:
  ...
  int& get_peer(int dim, Side side) { return m_peers(dim, side); }
  ...
  bool is_exchange_required(int dim)
  {
    ...
    return is_exchange_required(
      dim, halo_width, halo_width, halo_width, halo_width);
  }
