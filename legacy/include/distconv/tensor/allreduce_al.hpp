...
class AllreduceAl: public Allreduce<DataType> {
 public:
  ...
  AllreduceAl(std::shared_ptr<AlComm> comm_p): Allreduce<DataType>(),
      m_comm_p(comm_p), m_comm(*comm_p) {}
  virtual ~AllreduceAl() = default;

  virtual void allreduce(const DataType *send_buf, DataType *recv_buf,
                         size_t count) override {
    Al::Allreduce<AlBackend, DataType>(send_buf, recv_buf,
                                       count, Al::ReductionOperator::sum,
                                       m_comm);
  }
  virtual void allreduce(DataType *buf, size_t count) override {
    Al::Allreduce<AlBackend, DataType>(buf, count, Al::ReductionOperator::sum,
                                       m_comm);
  }
