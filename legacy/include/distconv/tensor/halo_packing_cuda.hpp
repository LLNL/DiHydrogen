...
#define HALO_EXCHANGE_ACCUME_OP_SWITCH(x)                                      \
  ...
  default: assert_always(0 && "Unknown accumulation op type");                 \
  }
...
struct PackFunctor
{
  ...
  template <typename T>
  __device__ typename std::enable_if<std::is_same<T, DataType>::value
  ...
  operator()(T& x, size_t offset)
  {
    ...
    {
      HaloExchangeAccumCUDAFunctor<T, op>()(x, ((T*) m_buf)[offset]);
    }
  }
};
...
#define CASE_BLOCK(OP)                                                         \
  case OP:                                                                     \
    pack_or_unpack<DataType, is_pack, PackFunctor<DataType, is_pack, OP>>(     \
...
struct PackAndPutBlockFunctor
{
  ...
  operator()(T const& x, size_t offset_base, int thread_offset, int num_elms)
  {
    ...
    util::nvshmem::put_nbi_block(
      ((T*) m_dst) + offset_base, ((T*) m_buf) + offset_base, num_elms, m_peer);
  }
};
...
struct PackPutNotifyBlockFunctor
{
  ...
  PackPutNotifyBlockFunctor(DataType* buf,
                            DataType* dst,
                            int peer,
                           ...
                           util::nvshmem::PairwiseSync& sync)
{
  ...
  TraverseHalo<TensorType, PackPutNotifyBlockFunctor<DataType>>(
    ...
    width,
    !is_reverse,
    PackPutNotifyBlockFunctor<DataType>(
      static_cast<DataType*>(buf), static_cast<DataType*>(dst), peer, sync),
    stream);
}
...
struct WaitAndUnpackFunctor
{
  ...
  __device__ void pre() { m_sync.wait(); }
};
