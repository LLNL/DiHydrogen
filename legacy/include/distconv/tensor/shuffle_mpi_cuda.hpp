        ...
        for (int pid = 0; pid < num_ranks; ++pid)
        {
            if (m_send_counts[pid] != 0 || m_recv_counts[pid] != 0)
            {
                util::MPIPrintStreamDebug()
                    << "Send/recv counts for " << pid << ": "
                    << m_send_counts[pid] << ", " << m_recv_counts[pid];
...
protected:
                           ...
                           int*& rank_limits)
    {
        ...
        for (int i = 0; i < num_dims; ++i)
        {
            ...
            for (int j = 0; j < dst_locale_dim; ++j)
            {
                ...
                {
                    lim = next_rank_global_offset
                          - src_tensor.get_global_index()[i];
                    // The offset of the next rank can be outside of this local
                    // tensor.
    ...
    virtual void release_buf(DataType* buf, h2::gpu::DeviceStream stream)
    {
        if (buf != nullptr && buf != m_src_buf && buf != m_dst_buf)
        {
            distconv::internal::RuntimeGPU::get_device_memory_pool().release(
                buf, stream);
        }
    }
