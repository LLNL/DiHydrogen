...
class TensorMPICUDAShufflerHybrid : public TensorMPICUDAShuffler<DataType>
{
    ...
    for (int i = 0; i < num_peers; ++i)
    {
      if (!m_conns[i])
      {
        ...
        util::MPIPrintStreamDebug()
          << "Shuffling with P2P not possible from rank "
          << this->m_loc.get_rank() << " to rank " << this->m_peers[i];
    ...
    for (int i = 0; i < num_peers; ++i)
    {
      ...
      else
      {
        ...
        Al::NonblockingSendRecv<Al::NCCLBackend, DataType>(
          ...
          this->get_recv_counts(is_forward)[peer],
          peer,
          m_al_comm,
          req);
      }
