...
class HaloExchangeHybrid:
 ...
 public:
  HaloExchangeHybrid(TensorType &tensor,
                     p2p::P2P &p2p)
      : HaloExchange<DataType, Allocator, AlBackend>(tensor),
        m_p2p(p2p), m_halo_peer(nullptr), m_p2p_enabled(false),
  ...
  void exchange(int dim,
    ...
    BoundaryAttributes<cudaStream_t> streams(
        comm_lhs->get_stream(), comm_rhs->get_stream());
    if (rendezvous) m_p2p.barrier(get_conns(dim), streams.data(), 2);
    for (auto side: SIDES) {
      ...
      this->pack_dim(dim, side, width_send, stream,
                     send_buf, is_reverse);
      // Use put if possible
      if (is_p2p_enabled(dim, side)) {
        get_conn(dim, side)->put(send_buf, get_halo_peer(dim, side),
                                 send_count * sizeof(DataType), stream);
      } else {
        // Use Al send/recv
    ...
    if (!skip_unpack) {
      this->unpack(dim, width_rhs_recv, width_lhs_recv,
                   comm_rhs->get_stream(), comm_lhs->get_stream(), is_reverse, op);
    }
    return;
  }
 ...
 protected:
  ...
  p2p::P2P::connection_type *get_conns(int dim) {
    return m_conns(dim);
  }

  void *&get_halo_peer(int dim, Side side) {
    return m_halo_peer(dim, side);
  }

  void **get_halo_peers(int dim) {
    return m_halo_peer(dim);
  }

  bool &is_p2p_enabled(int dim, Side side) {
    return m_p2p_enabled(dim, side);
  }
      ...
      for (auto side: SIDES) {
        if (get_conn(dim, side)) {
          ...
          util::MPIPrintStreamDebug()
              ...
              << this->m_tensor.get_locale().get_rank()
              << " to rank " << this->get_peer(dim, side);
