...
class Distribution {
 ...
 public:
  ...
  static Distribution make_distribution(
  ...
  static Distribution make_shared_distribution(
      std::initializer_list<int> locale_shape,
  ...
  int num_dims() const {
    return m_locale_shape.num_dims();
  }
  ...
  const Shape &get_locale_shape() const {
    return m_locale_shape;
  }

  index_t get_locale_shape(int dim) const {
    return m_locale_shape[dim];
  }
  ...
  const Shape &get_split_shape() const {
    return m_split_shape;
  }
    ...
    for (int i = 0; i < nd; ++i) {
      if (m_locale_shape[i] % m_split_shape[i] != 0) {
        util::PrintStreamError()
            << "m_locale_shape[" << i << "] not divisible by m_split_shape["
            << i << "]: " << m_locale_shape[i] << " % " <<
            m_split_shape[i];
  ...
  index_t get_block_size(int dim) const {
  ...
  bool is_shared(int dim) const {
    return get_num_ranks_per_split(dim) > 1;
  }
  ...
  bool is_split_root(const Array<ND> &rank) const {
    for (int i = 0; i < num_dims(); ++i) {
      if (!is_split_root(i, rank[i])) return false;
    }
    return true;
  }
