...
template <int ND, typename DataType,
          typename UnaryFunction, int BLOCK_SIZE>
__global__ static void reduce_kernel(
    const DataType *src,
    Array<ND> src_shape,
...
inline std::vector<int> find_reduce_dims(const Distribution &src_dist,
  ...
  for (int i = 0; i < nd; ++i) {
    ...
    // again.
    if (src_dist.get_split_shape()[i] !=
        src_dist.get_locale_shape()[i]) {
      continue;
    }
...
struct ReduceSumFunctor {
  ...
  int operator()(Tensor &src,
    ...
    if (local_reduction_shape.size() > 0) {
      ...
      reduce_kernel<ND, DataType, UnaryFunction, block_size>
          <<<grid_dims, block_dims, 0, stream>>>(
              src.get_const_base_ptr(),
              Array<ND>(local_reduction_shape), src_strides,
              dst.get_base_ptr(),
    ...
    // != locale_shape.
    std::vector<int> reduction_dims = find_reduce_dims(src.get_distribution(),
                                                       dst.get_distribution());
...
// assumes ND == 3 or 4
template <int ND, typename DataType,
          typename UnaryFunction1, typename UnaryFunction2, int BLOCK_SIZE>
__global__ static void reduce_kernel2(
    const DataType *src,
    Array<ND> src_shape,
    Array<ND> src_strides,
    DataType *dst1,
...
template <int ND, typename Tensor,
          typename UnaryFunction1, typename UnaryFunction2>
  ...
  int operator()(Tensor &src,
    ...
    if (local_reduction_shape.size() > 0) {
      ...
      reduce_kernel2<ND, typename Tensor::data_type, UnaryFunction1,
                     UnaryFunction2, block_size>
          <<<grid_dims, block_dims, 0, stream>>>(
              src.get_base_ptr(),
              Array<ND>(local_reduction_shape), src_strides,
              dst1.get_base_ptr(),
              dst1_shape, dst1_strides,
              op1,
              dst2.get_base_ptr(),
    ...
    std::vector<int> reduction_dims = find_reduce_dims(
        src.get_distribution(), dst1.get_distribution());
    dst1.allreduce(reduction_dims);
    reduction_dims = find_reduce_dims(
        src.get_distribution(), dst2.get_distribution());
...
template <int ND, typename DataType,
          typename Locale, typename Allocator>
...
ReduceSum(Tensor<DataType, Locale, Allocator> &src,
  ...
  return algorithms_cuda::ReduceSumFunctor<
    ND, TensorType, std::nullptr_t>()(src, src.get_local_shape(),
                                      dst, nullptr, stream);
}
...
ReduceSum(Tensor<DataType, Locale, Allocator> &src,
  ...
  using TensorType = Tensor<DataType, Locale, Allocator>;
  return algorithms_cuda::ReduceSumFunctor<
    ND, TensorType, std::nullptr_t>()(src, local_reduction_region,
                                      dst, nullptr, stream);
}
...
ReduceSum(Tensor<DataType, Locale, Allocator> &src,
  ...
  return algorithms_cuda::ReduceSumFunctor2<
    ND, TensorType, std::nullptr_t, std::nullptr_t>()(
