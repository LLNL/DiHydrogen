...
public:
  ...
  LocaleProcess get_sub_locale(int dim) const { return m_tensor->get_locale(); }

  index_t get_local_size() const { return m_tensor->get_size(); }

  Shape get_local_shape() const { return m_tensor->get_shape(); }
  ...
  Shape get_local_real_shape() const { return m_local_real_shape; }
  ...
  index_t get_dimension_rank_offset(int dim, int rank) const { return 0; }
...
protected:
  void update_local_real_shape()
  {
    ...
    {
      m_local_real_shape[i] = get_local_shape()[i] + dist.get_overlap(i) * 2;
    }
  }
  TensorType* m_tensor;
