...
class HaloExchangeNVSHMEM:
 ...
 public:
  ...
  void exchange(int dim,
    ...
    for (auto side: SIDES) {
      ...
      cudaStream_t stream = side == Side::RHS ? comm_rhs->get_stream() :
          comm_lhs->get_stream();
      exchange(dim, side, width_send, width_recv, stream,
               rendezvous, is_reverse, skip_unpack, op);
    }
    return;
  }
 ...
 protected:
  ...
  virtual void exchange(int dim, Side side,
                        int width_send, int width_recv,
                        cudaStream_t stream,
                        bool rendezvous,
                        bool is_reverse,
                        bool skip_unpack,
                        HaloExchangeAccumOp op)  {
    if (this->get_peer(dim, side) == MPI_PROC_NULL) return;
    if (rendezvous) {
      m_sync(dim, side).sync(this->get_peer(dim, side),
                             true, true, util::nvshmem::SyncType::FENCE,
                             stream);
    }
    if (width_send > 0) {
      ...
      nvshmemx_putmem_on_stream((void*)peer_recv_buf, send_buf,
    ...
    if (!skip_unpack && width_recv > 0) {
      ...
      this->unpack_dim(dim, side, width_recv, stream, recv_buf,
                       is_reverse, op);
    }
  }
};
...
class HaloExchangeNVSHMEMDirect:
 ...
 public:
  ...
  using CommType = typename HaloExchangeNVSHMEM<
    DataType, Allocator, AlBackend>::CommType;
  ...
  virtual void exchange(int dim, Side side,
    ...
    if (width_send > 0) {
      ...
      this->pack_and_put(dim, side, width_send, stream, send_buf, is_reverse,
                         peer_recv_buf, this->get_peer(dim, side));
    }
    this->m_sync(dim, side).sync(this->get_peer(dim, side),
                                 true, true, util::nvshmem::SyncType::FENCE,
                                 stream);
 ...
 protected:
  virtual void pack_and_put(int dim, Side side, int width,
                            cudaStream_t stream, void *buf,
                            bool is_reverse, void *dst, int peer);
};
 ...
 public:
  ...
  virtual void exchange(int dim, Side side,
    ...
    if (width_send > 0) {
      auto send_buf = this->get_send_buffer(dim, side);
      auto peer_recv_buf = this->get_recv_buffer(dim, ~side);
      this->pack_put_notify(dim, side, width_send, stream, send_buf,
                            is_reverse, peer_recv_buf,
                            this->get_peer(dim, side));
    } else {
    ...
    if (!skip_unpack && width_recv > 0) {
      auto recv_buf = this->get_recv_buffer(dim, side);
      this->wait_and_unpack(dim, side, width_recv, stream, recv_buf,
                            is_reverse, op);
    }
  }

 protected:
  virtual void pack_put_notify(int dim, Side side, int width,
                               cudaStream_t stream, void *buf,
                               bool is_reverse, void *dst, int peer);

  virtual void wait_and_unpack(int dim, Side side, int width, cudaStream_t stream,
...
class HaloExchangeNVSHMEMGraph:
 ...
 public:
  ...
  HaloExchangeNVSHMEMGraph(TensorType &tensor):
      HaloExchangeNVSHMEM<DataType, Allocator, AlBackend>(tensor),
      m_graph_ready(false) {}
  virtual ~HaloExchangeNVSHMEMGraph() {
    destroy_graph();
  }
 ...
 protected:
  ...
  virtual void exchange(int dim, Side side,
                        ...
                        HaloExchangeAccumOp op) override {
    if (!m_graph_ready(dim, side)) {
      DISTCONV_CHECK_CUDA(cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal));
      HaloExchangeNVSHMEM<DataType, Allocator, AlBackend>::exchange(
          dim, side, width_send, width_recv,
          stream, rendezvous, is_reverse,
          skip_unpack, op);
