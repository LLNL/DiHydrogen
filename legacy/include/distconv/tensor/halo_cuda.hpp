...
enum class HaloTraversalOpGroup
{
    THREAD,
    WARP,
    BLOCK
...
__global__
    typename std::enable_if<OpType::group == HaloTraversalOpGroup::THREAD,
                            void>::type
...
template <int ND, typename DataType, typename OpType>
__global__ typename std::enable_if<OpType::group == HaloTraversalOpGroup::BLOCK,
                                   void>::type
traverse_halo_generic_kernel(DataType* tensor,
                             Array<ND> shape,
                             int dim,
...
#define CALL_KERNEL(ND)                                                        \
    traverse_halo_generic_kernel<ND, DataType, OpType>                         \
        <<<grid_dims, block_dims, 0, s>>>(tensor,                              \
                                          Array<ND>(shape),                    \
                                          dim,                                 \
                                          side,                                \
                                          inner,                               \
...
__device__ static
    typename std::enable_if<OpType::group == HaloTraversalOpGroup::THREAD,
                            void>::type
    traverse_halo_opt_dim0_apply(DataType* tensor,
...
template <typename DataType, typename OpType>
__device__ static
    typename std::enable_if<OpType::group == HaloTraversalOpGroup::BLOCK,
                            void>::type
    traverse_halo_opt_dim0_apply(DataType* tensor,
                                 size_t tensor_offset,
                                 size_t packed_offset,
...
__device__ static void traverse_halo_opt_dim0(DataType* tensor,
                                              Array<ND> shape,
                                              int halo_width,
                                              OpType op)
{
    ...
    for (int ch_idx = blockIdx.x; ch_idx < shape[-2]; ch_idx += gridDim.x)
    {
        ...
        {
            size_t packed_offset =
                offset_common * halo_width + packed_halo_idx - threadIdx.x;
            size_t tensor_offset =
                offset_common * shape[0] + halo_idx - threadIdx.x;
            traverse_halo_opt_dim0_apply(tensor,
                                         tensor_offset,
                                         packed_offset,
                                         shape[0],
                                         shape[1],
                                 ...
                                 OpType op)
{
    for (int x = 0; x < len; x += blockDim.x)
    {
        ...
        op(tensor[tensor_offset + tensor_idx],
           packed_offset + x,
           threadIdx.x,
           min(blockDim.x, len - x));
    }
}
                                    ...
                                    OpType op)
{
    ...
    {
        int const z_block_len =
            min(static_cast<int>(blockDim.y),
                z_len - (y - static_cast<int>(threadIdx.y)));
#pragma unroll
        for (int hw = 0; hw < halo_width; ++hw)
        {
            size_t packed_offset =
                packed_offset_common + x_len * z_block_len * hw;
...
#pragma unroll
        for (int hw = 0; hw < halo_width; ++hw)
        {
            size_t packed_offset = packed_offset_common
                                   + x_len * z_block_len * hw
                                   + z * x_len * halo_width;
            int halo_idx = hw + halo_tensor_base;
            size_t tensor_offset = tensor_offset_common + x_len * halo_idx
            ...
            for (int x = 0; x < x_len; x += blockDim.x)
            {
                ...
                op(tensor[tensor_offset + x_thread],
                   packed_offset + x,
                   tid,
                   min(blockDim.x, x_len - x) * z_block_len);
            }
        }
    }
}
...
#pragma unroll
            for (int hw = 0; hw < halo_width; ++hw)
            {
                ...
                size_t tensor_offset =
                    tensor_offset_common + shape[0] * halo_idx;
                traverse_halo_opt_dim1_apply(
                    tensor, tensor_offset, packed_offset, shape[0], op);
            }
        }
        offset_common += ch_offset;
    }
    ...
    for (int y = 0; y < y_len; y += blockDim.y)
    {
        for (int x = 0; x < x_len; x += blockDim.x)
        {
            ...
            op(tensor[tensor_offset_common + tensor_idx],
               ...
               tid,
               min(blockDim.x, x_len - x) * min(blockDim.y, y_len - y));
        }
    }
}
...
#pragma unroll
        for (int hw = 0; hw < halo_width; ++hw)
        {
            traverse_halo_opt_dim2_apply(tensor,
                                         tensor_offset_common,
                                         packed_offset_common,
                                         shape[0],
                                         shape[1],
...
__device__ static typename std::enable_if<OpType::has_pre_grid, void>::type
...
template <typename OpType>
__device__ static typename std::enable_if<OpType::has_post_grid, void>::type
...
traverse_halo_opt(DataType* __restrict__ tensor, Array<ND> shape, OpType op)
{
...
template <int ND,
          ...
          Side side,
          int dim>
inline void traverse_halo_opt(DataType* tensor,
                              Array<ND> shape,
                              int halo_width,
                              OpType op,
                              h2::gpu::DeviceStream s,
...
#define CALL(HW)                                                               \
    ...
    {                                                                          \
        ...
        DISTCONV_CHECK_GPU(                                                    \
            GPU_LAUNCH_COOP_KERNEL((const void*) (traverse_halo_opt<ND,        \
                                                                    DataType,  \
                                                                    OpType,    \
                                                                    inner,     \
                                                                    side,      \
    ...
    {                                                                          \
        ...
        {                                                                      \
            util::MPIPrintStreamError()                                        \
                << "Lauch error: " << gsize.x << "x" << gsize.y << "x"         \
                << gsize.z << ", " << bsize.x << "x" << bsize.y << "x"         \
                << bsize.z;                                                    \
    ...
    default:
        ...
        util::MPIRootPrintStreamError()
            << "Unsupported halo size: " << halo_width;
        throw std::exception();
    }
#undef CALL
}
                              ...
                              h2::gpu::DeviceStream s)
{
    ...
    else if (dim == 1)
    {
        ...
        if (ND == 5)
            assert_always(shape[0] <= max_dim);
        bsize.x = std::min(shape[0], max_dim);
        if (ND == 5)
            bsize.y = std::min(shape[2], max_dim / bsize.x);
    }
    else if (dim == 2)
    {
        bsize.x = std::min(shape[0], max_dim);
    ...
    if (vector_width == 2)
    {
        ...
        traverse_halo_opt<ND, VecType, OpType, inner, side, dim>(
            (VecType*) tensor, shape, halo_width, op, s, gsize, bsize);
    }
    else
    {
        traverse_halo_opt<ND, DataType, OpType, inner, side, dim>(
...
void TraverseHalo(Tensor& tensor,
                  ...
                  h2::gpu::DeviceStream s)
{
    TraverseHalo(tensor, dim, width_rhs, Side::RHS, inner, op, s);
    TraverseHalo(tensor, dim, width_lhs, Side::LHS, inner, op, s);
}

template <typename Tensor, typename OpType>
void TraverseHalo(
    Tensor& tensor, int dim, bool inner, OpType op, h2::gpu::DeviceStream s)
{
