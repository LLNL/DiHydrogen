                                              ...
                                              OpType op)
{
  ...
  for (int ch_idx = blockIdx.x; ch_idx < shape[-2]; ch_idx += gridDim.x)
  {
    ...
    else
    {
      ...
      size_t tensor_offset = offset_common * shape[0] + halo_idx - threadIdx.x;
                                  ...
                                  OpType op)
{
  ...
  {
    int const z_block_len = min(static_cast<int>(blockDim.y),
...
#pragma unroll
    for (int hw = 0; hw < halo_width; ++hw)
    {
      ...
      int halo_idx = hw + halo_tensor_base;
      size_t tensor_offset =
        tensor_offset_common + x_len * halo_idx + z_thread * x_len * y_len;
  ...
  {                                                                            \
    ...
    {                                                                          \
      util::MPIPrintStreamError()                                              \
        << "Lauch error: " << gsize.x << "x" << gsize.y << "x" << gsize.z      \
