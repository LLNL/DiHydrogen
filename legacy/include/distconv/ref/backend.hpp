...
class Backend {
 public:
  ...
  struct filter {
    static void apply(typename Tensor::data_type alpha,
                      const Tensor &x,
...
class Convolution<ref::Backend, ND, DataType> {
 public:
    ...
    for (index_t n = 0; n < input.get_local_shape()[3]; ++n) {
      for (index_t k = 0; k < output.get_local_shape()[2]; ++k) {
        for (index_t c = 0; c < input.get_local_shape()[2]; ++c) {
          ref::Backend::filter<ND, Tensor>::apply(alpha, input, n, c,
                                                  filter, k, c, false,
    ...
    for (index_t n = 0; n < d_output.get_local_shape()[3]; ++n) {
      for (index_t k = 0; k < d_output.get_local_shape()[2]; ++k) {
        for (index_t c = 0; c < filter.get_local_shape()[2]; ++c) {
          ref::Backend::filter<ND, Tensor>::apply(alpha, d_output, n, k,
                                                  filter, k, c, true,
