...
inline HaloExchangeMethod GetHaloExchangeMethod(std::string const& method)
{
  ...
  else
  {
    util::PrintStreamError()
      << "Unknown method name for halo exchange: " << method;
    std::abort();
  }
}
...
inline bool IsNVSHMEMUsed(HaloExchangeMethod m)
{
  ...
  case HaloExchangeMethod::NVSHMEM_FUSED_NOTIFY: return true;
...
enum class ChannelParallelismAlgorithm
{
  NONE,
  AUTO,
  X,
...
inline BatchnormImpl GetBatchnormImpl(std::string const& impl)
{
  ...
  {
    util::PrintStreamError()
      << "Unknown implementation name for batchnorm: " << impl;
    std::abort();
  }
}
...
inline bool IsNVSHMEMUsed(BatchnormImpl m)
{
  ...
  case BatchnormImpl::FUSED_NVSHMEM_RECURSIVE_DOUBLING: return true;
  default: return false;
  }
#else
  return false;
