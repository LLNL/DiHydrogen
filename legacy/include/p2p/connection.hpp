...
public:
  Connection(int peer, const internal::MPI& mpi, util::EventPool& ev_pool);
  ...
  virtual int send(const void* buf, size_t size, cudaStream_t stream) = 0;
  virtual int recv(void* buf, size_t size, cudaStream_t stream) = 0;
  virtual int sendrecv(const void* send_buf,
                       size_t send_size,
                       void* recv_buf,
                       size_t recv_size,
  ...
  virtual int
  put(const void* src, void* dst, size_t size, cudaStream_t stream) = 0;
...
protected:
  ...
  void add_or_replace_mapped_peer_memory(const void* peer, void* mapped_addr);
