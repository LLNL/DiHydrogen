    ...
    for (index_t j = 0; j < local_shape[1]; ++j)
    {
      for (index_t i = threadIdx.x; i < local_shape[0]; i += blockDim.x)
      {
        ...
        if (stored != global_offset)
        {
          ...
          printf("Error at (%d, %d, %d); ref: %d, stored: %f, "
                 "global_index_base(%d, %d, %d)\n",
                 (int) global_idx[0],
                 (int) global_idx[1],
                 (int) global_idx[2],
                 global_offset,
      ...
      for (index_t j = 0; j < local_shape[1]; ++j)
      {
        for (index_t i = threadIdx.x; i < local_shape[0]; i += blockDim.x)
        {
          ...
          if (stored != global_offset)
          {
            atomicAdd(error_counter, 1);
#if 1
            printf("Error at (%d, %d, %d, %d); ref: %d, stored: %f, "
                   "global_index_base(%d, %d, %d, %d), local_offset: %d\n",
                   (int) global_idx[0],
                   (int) global_idx[1],
                   (int) global_idx[2],
                   (int) global_idx[3],
...
#if 1
              printf(
                "Error at (%d, %d, %d, %d, %d); ref: %d, stored: %f, "
                "global_index_base(%d, %d, %d, %d, %d), local_offset: %d\n",
                (int) global_idx[0],
                (int) global_idx[1],
                (int) global_idx[2],
                (int) global_idx[3],
                      ...
                      ShuffleMethod method)
{
  ...
  case ShuffleMethod::AL:
    ...
    shuffler =
      new tensor::TensorMPICUDAShufflerAL<DataType>(t_src, t_dest, *al_comm);
    break;
#ifdef DISTCONV_HAS_P2P
  case ShuffleMethod::P2P:
    p2p_h = new p2p::P2P(MPI_COMM_WORLD);
    shuffler = new TensorMPICUDAShufflerP2P<DataType>(t_src, t_dest, *p2p_h);
    break;
  ...
  {
    check_tensor<ND><<<grid_dim, block_dim>>>(t_dest.get_buffer(),
  ...
  {
    check_tensor<ND><<<grid_dim, block_dim>>>(t_src.get_buffer(),
                                              t_src.get_local_shape(),
                                              t_src.get_overlap(),
                                              t_src.get_pitch(),
                                              t_src.get_shape(),
...
int run_tests(const Shape& proc_dim, const Shape& shape, ShuffleMethod method)
{
...

  {
    MPI_Barrier(MPI_COMM_WORLD);
    util::MPIRootPrintStreamInfo()
      << "Test: shrink distribution of the spatial dimensions from "
         "sample-parallel tensors.";
    auto split_shape = proc_dim;
    for (int i = 0; i < NSD; i++)
      split_shape[i] = 1;
    Distribution dist1(proc_dim,
                       split_shape,
                       create_spatial_overlap(),
    ...
    if (!skip)
    {
      ...
      assert_always((test_copy_shuffle<ND, TensorMPI, TensorMPI>(
                      shape, dist1, sample_dist, method))
                    == 0);
      MPI_Barrier(MPI_COMM_WORLD);
      assert_always((test_copy_shuffle<ND, TensorMPI, TensorMPI>(
                      shape, sample_dist, dist1, method))
                    == 0);
    }
  }
