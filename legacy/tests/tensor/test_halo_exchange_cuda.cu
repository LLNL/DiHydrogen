...
#if 1
              printf("Error at (%d, %d, %d, %d), dir: %d; ref: %lu, stored: %f, global_index_base(%d, %d, %d, %d), local_index(%d, %d, %d, %d), local offset: %d\n",
          ...
          for (index_t i = threadIdx.x; i < halo_shape[0]; i += blockDim.x) {
            for (int dir = 0; dir < 2; ++dir) {
              ...
              if (stored != global_offset) {
                atomicAdd(error_counter, 1);
#if 1
                printf("Error at (%d, %d, %d, %d, %d), dir: %d; ref: %lu, stored: %f, global_index_base(%d, %d, %d, %d, %d), local_index(%d, %d, %d, %d, %d), local offset: %d\n",
                       (int)global_idx[0], (int)global_idx[1],
                       (int)global_idx[2], (int)global_idx[3],
                       (int)global_idx[4],
                       dir,
  ...
  for (index_t l = 0; l < boundary_shape[3]; ++l) {
    for (index_t k = 0; k < boundary_shape[2]; ++k) {
      for (index_t j = 0; j < boundary_shape[1]; ++j) {
        for (index_t i = threadIdx.x; i < boundary_shape[0];
             i += blockDim.x) {
          for (int dir = 0; dir < 2; ++dir) {
            Array<4> local_idx = {i, j, k, l};
...
int test_halo_exchange(const Array<ND> &shape,
  ...
  MPI_Allreduce(MPI_IN_PLACE, &is_too_small,
...
int test_halo_exchange_reverse(const Array<ND> &shape,
  ...
  halo_exc->exchange(comms, stream_main,
  ...
  for (int i = 0; i < dims.size(); ++i) {
    if (tensor.get_local_size() > 0) {
      check_tensor_reverse<ND><<<1, block_dim>>>(
          tensor.get_buffer(),
          tensor.get_local_shape(),
          dist.get_overlap(),
          tensor.get_pitch(),
...
template <int ND, typename Tensor>
int dispatch_tests(int pid, int np,
                   const Shape &proc_dim,
                   const Shape &tensor_shape,
                   HaloExchangeMethod method) {
  const auto create_spatial_overlap =
      [](const Shape &proc_dim, const int size) {
...
#if 1
  {
    ...
    run_test<ND, Tensor>(pid, np, tensor_shape, method,
                         create_spatial_overlap(proc_dim, 1));
  }
#endif
...
#if 1
  {
    ...
    run_test<ND, Tensor>(pid, np, tensor_shape, method,
                         create_spatial_overlap(proc_dim, 2));
  }
#endif
...
int main(int argc, char *argv[]) {
  ...
  std::vector<HaloExchangeMethod> methods = {
    HaloExchangeMethod::MPI, HaloExchangeMethod::AL,
#ifdef DISTCONV_HAS_P2P
    HaloExchangeMethod::P2P, HaloExchangeMethod::HYBRID
#endif // DISTCONV_HAS_P2P
  };
  ...
  if (argc > 0) {
    ...
    if (name == "MPI") {
      ...
      util::MPIRootPrintStreamError() << "Unknown halo exchange method name: "
                                      << name;
      MPI_Finalize();
      exit(1);
    }
