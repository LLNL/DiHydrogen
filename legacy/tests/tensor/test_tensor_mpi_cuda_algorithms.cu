...
inline int test_transform(const Shape& shape, const Distribution& dist)
{
  ...
  using TensorTypeLong = Tensor<long,
  ...
  Transform(t2,
            t3,
            [] __device__(typename TensorType::data_type & x,
                       ...
                       const Distribution& dist)
{
  ...
  {
    assert_always(shape[i] == reduce_shape[i] || reduce_shape[i] == 1);
  }
  ...
  TensorType reduce_tensor =
    get_tensor<TensorType>(reduce_shape, loc, reduce_dist);
  ...
  using TensorProcType =
    Tensor<DataType, tensor::LocaleProcess, tensor::BaseAllocator>;
  ...
  TensorType single_tensor =
    get_tensor<TensorType>(Shape(num_dims, 1), loc, reduce_dist);
  assert0(single_tensor.allocate());
  single_tensor.zero();
  util::MPIRootPrintStreamDebug() << "Single-element tensor: " << single_tensor;
  ReduceSum<ND, DataType, LocaleType, typename TensorType::allocator_type>(
                                 ...
                                 const UnaryFunction& op)
{
  ...
  TransformReduceSum<ND,
                     DataType,
                     LocaleType,
                     typename TensorType::allocator_type>(t, reduce_tensor, op);
...
#else
  auto proc_dist = Distribution::make_localized_distribution(num_dims);
  TensorProcType input_tensor_host(LocaleProcess(), proc_dist);
  assert0(tensor::Copy(input_tensor_host, t, 0));
#endif
  TensorProcType reduce_tensor_host(LocaleProcess(), proc_dist);
  assert0(tensor::Copy(reduce_tensor_host, reduce_tensor, 0));
  int num_errors = 0;
  if (loc.get_rank() == 0)
  {
                                  ...
                                  const UnaryFunction2& op2)
{
  ...
  TensorType reduce_tensor1 =
    get_tensor<TensorType>(reduce_shape1, loc, reduce_dist1);
  ...
  TensorType reduce_tensor2 =
    get_tensor<TensorType>(reduce_shape2, loc, reduce_dist2);
  ...
  if (loc.get_rank() == 0)
  {
    ...
    for (auto it = reduce_tensor_host1.get_shape().index_begin();
         it != reduce_tensor_host1.get_shape().index_end();
         ++it)
    {
  ...
  if (loc.get_rank() == 0)
  {
    ...
    for (auto it = reduce_tensor_host2.get_shape().index_begin();
         it != reduce_tensor_host2.get_shape().index_end();
         ++it)
    {
...
int main(int argc, char* argv[])
{
  ...
  for (auto it = reduced_dim.index_begin(); it != reduced_dim.index_end(); ++it)
  {
  ...
  for (auto it = reduced_dim.index_begin(); it != reduced_dim.index_end(); ++it)
  {
    ...
    assert0(test_transform_reduce<num_dims, TensorMPI>(
      tensor_shape, reduce_shape, dist, [] __host__ __device__(int x) {
        return x * 2;
      }));
  }
