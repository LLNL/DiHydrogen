...
int test_view_raw_ptr(const Shape &shape,
  ...
  for (index_t i = 0; i < local_shape[2]; ++i) {
    for (index_t j = 0; j < local_shape[1]; ++j) {
      for (index_t k = 0; k < local_shape[0]; ++k) {
        IndexVector idx({k, j, i});
        index_t x = get_linearlized_offset(t.get_global_index(idx),
                                           t.get_shape());
  ...
  for (index_t i = 0; i < local_shape[2]; ++i) {
    for (index_t j = 0; j < local_shape[1]; ++j) {
      for (index_t k = 0; k < local_shape[0]; ++k) {
        IndexVector idx({k, j, i});
        int ref = get_linearlized_offset(t.get_global_index(idx),
                                         t.get_shape());
...
int test_copy(const Shape &shape, const Distribution &dist_src,
              const Distribution &dist_dest, int root) {
  assert_eq(shape.num_dims(), 3);
...
int main(int argc, char *argv[]) {
  ...
  util::MPIRootPrintStreamInfo() << "test_data_access with a shared tensor success";

  assert0(test_view<TensorProc, TensorMPI>(Shape({2, 2, 2}),
                                           dist, shared_dist));
  ...
  assert0(test_copy<TensorProc, TensorMPI>(Shape({2, 2, 4}),
                                           dist, shared_dist, 0));
