  ...
  if (peer >= 0 && peer < np)
  {
    ...
    if ((pid % 2) == 0)
    {
      ...
      P2P_CHECK_CUDA_ALWAYS(
        cudaMemcpy(buf1, host, sizeof(int) * len, cudaMemcpyHostToDevice));
    ...
    P2P_CHECK_CUDA_ALWAYS(
      cudaMemcpy(host, buf1, sizeof(int) * len, cudaMemcpyDeviceToHost));
    for (int i = 0; i < len; ++i)
    {
      if (host[i] != i)
      {
...
int test_sendrecv(int pid, int np, p2p::P2P& p2p)
{
  ...
  P2P_CHECK_CUDA_ALWAYS(
    cudaMemcpy(buf1, host, sizeof(int) * len, cudaMemcpyHostToDevice));
  conn->sendrecv(buf1, sizeof(int) * len, buf1, sizeof(int) * len, st);
  P2P_CHECK_CUDA_ALWAYS(cudaStreamSynchronize(st));
  P2P_CHECK_CUDA_ALWAYS(
    cudaMemcpy(host, buf1, sizeof(int) * len, cudaMemcpyDeviceToHost));
  for (int i = 0; i < len; ++i)
  {
    ...
    {
      p2p::logging::MPIPrintStreamError()
        << "Mismatch at " << i << ": host[i]=" << host[i] << " != " << i + peer
        << "\n";
      return 1;
    }
  }
