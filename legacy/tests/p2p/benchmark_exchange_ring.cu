...
int bandwidth_put(int pid, int np, p2p::P2P &p2p,
  ...
  for (int i = 0; i < 2; ++i) {
    P2P_CHECK_CUDA_ALWAYS(cudaMemcpy(bufs[i], reference, max_size,
  ...
  for (size_t size = min_size; size <= max_size;
    ...
    for (int i = 0; i < iter + skip; ++i) {
      ...
      p2p.exchange(conns, (void **)bufs, (void **)dst_bufs, remote_bufs,
                   sizes, sizes, streams, 2);
    }
    ...
    float elapsed;
    P2P_CHECK_CUDA(cudaEventElapsedTime(&elapsed,
    ...
    if (validate) {
      ...
      for (int i = 0; i < 2; ++i) {
        P2P_CHECK_CUDA_ALWAYS(cudaMemcpy(host, dst_bufs[i], size,
...
int main(int argc, char *argv[]) {
  ...
  TEST_RUN(bandwidth_put(pid, np, p2p, min_size, max_size,
                         measurement_pid, iter, validate));
