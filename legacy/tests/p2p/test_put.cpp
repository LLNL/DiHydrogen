...
int put_neighbor(int pid, int np, p2p::P2P &p2p, int count,
                 bool wrap_around) {
  ...
  P2P_CHECK_CUDA_ALWAYS(cudaMemcpy(send_rhs, host, sizeof(int) * len,
                                   cudaMemcpyHostToDevice));
  P2P_CHECK_CUDA_ALWAYS(cudaMemcpy(send_lhs, host, sizeof(int) * len,
                                   cudaMemcpyHostToDevice));
  ...
  P2P_CHECK_CUDA_ALWAYS(cudaMemcpyAsync(host_rhs, recv_rhs, sizeof(int) * len,
                                        cudaMemcpyDeviceToHost, st));
  P2P_CHECK_CUDA_ALWAYS(cudaMemcpyAsync(host_lhs, recv_lhs, sizeof(int) * len,
                                        cudaMemcpyDeviceToHost, st));
  ...
  for (int i = 0; i < len; ++i) {
    if (rhs != MPI_PROC_NULL) {
      if (host_rhs[i] != i + len * rhs) {
        p2p::logging::MPIPrintStreamInfo() << "RHS Mismatch at " << i
                                           << "; " << host_rhs[i] << " != " << i + len * rhs
                                           << "\n";
      }
    }
    if (lhs != MPI_PROC_NULL) {
      if (host_lhs[i] != i + len * lhs) {
        p2p::logging::MPIPrintStreamInfo() << "LHS Mismatch at " << i
                                           << "; " << host_lhs[i] << " != " << i + len * lhs
                                           << "\n";
      }
    }
  }
