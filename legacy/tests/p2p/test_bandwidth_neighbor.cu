                  ...
                  int measurement_pid)
{
  ...
  P2P_CHECK_CUDA_ALWAYS(
    cudaDeviceGetStreamPriorityRange(&stream_min_priority, nullptr));
  // P2P_CHECK_CUDA_ALWAYS(cudaStreamCreate(&st));
  P2P_CHECK_CUDA_ALWAYS(
    cudaStreamCreateWithPriority(&st, cudaStreamDefault, stream_min_priority));
#ifdef USE_SEPARATE_STREAMS
  cudaStream_t st2;
  P2P_CHECK_CUDA_ALWAYS(
    cudaStreamCreateWithPriority(&st2, cudaStreamDefault, stream_min_priority));
#else
  cudaStream_t st2 = st;
#endif
  ...
  for (size_t size = min_size; size <= max_size; size *= 2)
  {
    ...
    float elaplsed;
    P2P_CHECK_CUDA(cudaEventElapsedTime(&elaplsed, timing_start, timing_end));
    if (measurement_pid == pid)
    {
      std::stringstream ss;
      ss << pid << " " << size << " " << elaplsed * 1000 << "\n";
      std::cout << ss.str();
      std::cout << std::flush;
    }
  }
...
int main(int argc, char* argv[])
{
  ...
  TEST_RUN(
    bandwidth_put(pid, np, p2p, min_size, max_size, false, measurement_pid));
  TEST_RUN(
    bandwidth_put(pid, np, p2p, min_size, max_size, true, measurement_pid));
