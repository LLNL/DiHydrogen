...
write_s5d(std::basic_ostream<C, T>& os, const s5d& s, bool ignore_first = false)
{
  ...
  return os << std::get<0>(s) << '_' << std::get<1>(s) << '_' << std::get<2>(s)
            << '_' << std::get<3>(s) << '_' << std::get<4>(s);
}
                                           ...
                                           const ConvParams& p)
{
  return os << p.pads[0] << '_' << p.pads[1] << '_' << p.pads[2] << '_'
            ...
            << p.dilation[0] << '_' << p.dilation[1] << '_' << p.dilation[2]
            << '_' << p.groups;
}
  ...
  Stream_t const stream) const
{
  ...
  {
    // Try to invoke a JIT-compiled convolution
    bool jit_compiled = invoke(desc, x, dw, dy, alpha, beta, workspace, stream);
    if (jit_compiled)
      return;
  }
  ...
  ConvFwdAlgo_t const& algo) const
{
  ConvDescriptor desc;
  desc.type = FORWARD;
  if (descriptor_from_tensors(in_desc, filter_desc, conv_desc, out_desc, desc))
  {
                                         ...
                                         bool dynamic_minibatch_size) const
{
  ...
  if (handle)
  {
    ...
    {  // In case of no workspace requirements, the functions will not exist
      ...
      {
        result.dynbatch_get_ws_size = (dynbatch_getworkspacesize_t) getwssize;
