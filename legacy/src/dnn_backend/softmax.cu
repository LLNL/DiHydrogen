...
struct id
{
  __device__ __forceinline__ DataType operator()(DataType x) const { return x; }
};
...
struct atomic_max<double>
{
  __device__ __forceinline__ double operator()(double* addr, double value) const
  {
    ...
    do
    {
      ...
      old = atomicCAS(
        (unsigned long long*) addr_as_ll, assumed, (__double_as_longlong(m)));
    } while (assumed != old);
                     ...
                     h2::gpu::DeviceStream stream)
{
  ...
  update_per_sample_kernel<DataType, block_size, SoftmaxOp<DataType>>
    <<<gdim, block_size, 0, stream>>>(output_tensor.get_base_ptr(),
                                      sample_exp,
                                      sample_size,
...
int Softmax<BackendDNNLib>::forward(const Tensor& x, Tensor& y)
{
  ...
  DataType* sample_max = static_cast<DataType*>(mempool.get(ws_size, m_stream));
  DataType* sample_exp = static_cast<DataType*>(mempool.get(ws_size, m_stream));
                                     ...
                                     Tensor& dx)
{
  ...
  DataType* sample_dp = static_cast<DataType*>(mempool.get(ws_size, m_stream));
...
#define PROTO(T)                                                               \
  template int Softmax<BackendDNNLib>::forward<TensorCUDA<T>>(                 \
    const TensorCUDA<T>& x, TensorCUDA<T>& y);                                 \
  template int Softmax<BackendDNNLib>::backward<TensorCUDA<T>>(                \
