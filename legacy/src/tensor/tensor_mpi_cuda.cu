...
struct ClearHaloFunctor
{
  ...
  ClearHaloFunctor() {}
  __device__ void operator()(DataType& x, size_t) { x = DataType(0); }
...
struct ScaleFunctor
{
  ...
  __device__ void operator()(DataType& x) { x *= m_v; }
  DataType m_v;
};
...
template <int ND,
          ...
          typename DataType1,
          typename DataType2>
__global__ void concat_or_slice_kernel(DataType1* dst,
                                       Array<ND> dst_shape,
                                       Array<ND> dst_strides,
                                       DataType2* src1,
                                       Array<ND> src1_shape,
                                       Array<ND> src1_strides,
                                       DataType2* src2,
                                       Array<ND> src2_shape,
                                       Array<ND> src2_strides,
                                       int concat_dim)
{
...
int ConcatenateOrSlice(
  typename AddConstIf<!IS_CONCAT,
                      Tensor<DataType, LocaleMPI, CUDAAllocator>>::type& t_dest,
  typename AddConstIf<IS_CONCAT,
                      Tensor<DataType, LocaleMPI, CUDAAllocator>>::type& t_src1,
  typename AddConstIf<IS_CONCAT,
...
#define CALL_KERNEL(ND, INNER_DIM)                                             \
  do                                                                           \
  {                                                                            \
    ...
    concat_or_slice_kernel<ND, INNER_DIM, IS_CONCAT>                           \
      <<<grid_dim, block_dim, 0, s>>>(t_dest.get_base_ptr(),                   \
                                      Array<ND>(t_dest.get_local_shape()),     \
                                      Array<ND>(t_dest.get_strides()),         \
                                      t_src1.get_base_ptr(),                   \
                                      Array<ND>(t_src1.get_local_shape()),     \
                                      Array<ND>(t_src1.get_strides()),         \
                                      t_src2.get_base_ptr(),                   \
                                      Array<ND>(t_src2.get_local_shape()),     \
  ...
  switch (nd)
  {
  case 3: CALL_KERNEL(3, 1); break;
  case 4: CALL_KERNEL(4, 1); break;
  case 5:
