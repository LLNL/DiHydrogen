...
int MPI::send(void const* buf, size_t size, int dst)
{
  ...
  logging::MPIPrintStreamDebug()
    << "MPI_Send, size: " << size << ", dst: " << dst << ", tag: " << MPI::m_tag
    << "\n";
#endif
...
#ifdef P2P_MPI_LOGGING_DEBUG
  logging::MPIPrintStreamDebug()
    << "MPI_Recv, size: " << size << ", src: " << src << ", tag: " << MPI::m_tag
    << "\n";
#endif
  MPI_Recv(buf, size, MPI_BYTE, src, MPI::m_tag, get_comm(), MPI_STATUS_IGNORE);
  return 0;
}

int MPI::irecv(void* buf, size_t size, int src, MPI_Request* req)
{
#ifdef P2P_MPI_LOGGING_DEBUG
  logging::MPIPrintStreamDebug()
    << "MPI_Irecv, size: " << size << ", src: " << src
    << ", tag: " << MPI::m_tag << "\n";
#endif
  MPI_Irecv(buf, size, MPI_BYTE, src, MPI::m_tag, get_comm(), req);
  return 0;
}
...
int MPI::barrier(int peer)
{
  int x = 0;
  int y;
  return sendrecv(&x, sizeof(int), peer, &y, sizeof(int), peer);
}
...
#ifdef P2P_MPI_LOGGING_DEBUG
  logging::MPIPrintStreamDebug()
    << "MPI_Waitall with " << num_requests << " requests\n";
#endif
