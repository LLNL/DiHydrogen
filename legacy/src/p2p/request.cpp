...
namespace p2p {
Request::Request(): Request(Kind::NULL_REQUEST, nullptr,
                            nullptr, 0, 0, nullptr) {}

Request::Request(Connection *conn, MPI_Request req,
                 cudaStream_t stream):
    Request(Kind::DEFAULT, conn, req, stream) {}

Request::Request(Kind kind, Connection *conn, MPI_Request req,
                 cudaStream_t stream):
    Request(kind, conn, &req, 1, stream, nullptr) {}

Request::Request(Connection *conn, MPI_Request req1,
                 MPI_Request req2, cudaStream_t stream):
    Request(Kind::DEFAULT, conn, req1, req2, stream) {}

Request::Request(Kind kind, Connection *conn, MPI_Request req1,
                 MPI_Request req2, cudaStream_t stream):
...
Request::Request(Connection *conn, MPI_Request *mpi_requests,
                 int num_requests, cudaStream_t stream,
                 handler_type handler):
    Request(Kind::DEFAULT, conn, mpi_requests, num_requests,
            stream, handler) {}

Request::Request(Kind kind, Connection *conn, MPI_Request *mpi_requests,
                 int num_requests, cudaStream_t stream,
                 handler_type handler):
    m_kind(kind), m_conn(conn), m_num_requests(num_requests),
...
Request::Request(const Request &req):
    m_kind(req.m_kind), m_conn(req.m_conn),
    m_num_requests(req.m_num_requests), m_stream(req.m_stream),
    m_data(req.m_data), m_handler(req.m_handler) {
  for (int i = 0; i < MAX_MPI_REQUESTS; ++i) {
...
bool Request::run_post_process(Request *req) {
  ...
  if (m_handler) {
    ...
    switch (m_kind) {
      case Kind::CONNECT:
        ret = m_conn->connect_post();
        break;
      case Kind::REGISTER:
        ret = m_conn->register_addr_post(m_data);
        break;
      case Kind::NOTIFY:
        ret = m_conn->notify_post(m_stream);
        break;
      case Kind::WAIT:
        ret = m_conn->wait_post(m_stream);
        break;
...
int Request::add_mpi_request(MPI_Request req) {
  if (m_num_requests + 1 <= MAX_MPI_REQUESTS) {
    ...
    logging::MPIPrintStreamError() << "Can't add MPI_Request as the capacity in Request is full\n";
    return 1;
  }
}
...
int Request::wait(const Request *requests, int num_requests,
                  internal::MPI &mpi) {
  int num_mpi_req = 0;
...
void Request::process(const Request *requests, int num_requests,
                      internal::MPI &mpi) {
