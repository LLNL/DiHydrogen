                             ...
                             std::vector<char*>& names)
{
  ...
  {
    MPI_Request mr;
    m_mpi.isend(m_proc_name, MPI_MAX_PROCESSOR_NAME, peers[i], &mr);
    requests.push_back(mr);
    char* peer_name = new char[MPI_MAX_PROCESSOR_NAME];
    m_mpi.irecv(peer_name, MPI_MAX_PROCESSOR_NAME, peers[i], &mr);
    requests.push_back(mr);
                          ...
                          std::vector<int>& peer_devices)
{
  ...
  for (int i = 0; i < num_peers; ++i)
  {
    ...
    logging::MPIPrintStreamDebug()
      << "sending device " << m_dev << " to " << peers[i] << "\n";
    m_mpi.isend(&m_dev, sizeof(int), peers[i], &mr);
    requests.push_back(mr);
    logging::MPIPrintStreamDebug()
      << "receiving device from " << peers[i] << "\n";
                         ...
                         int num_peers)
{
  ...
  MPIPrintStreamDebug() << requests.size() << " new connections\n";
  Request::process(requests.data(), requests.size(), m_mpi);
  return 0;
}

std::shared_ptr<Connection>
P2P::connect(int peer, char* peer_name, int peer_dev)
{
                        ...
                        std::vector<void*>& peer_addrs)
{
  ...
  return exchange_addrs(
    connections.data(), local_addrs.data(), peer_addrs.data(), num_conns);
}
                        ...
                        int num_conns)
{
  ...
  {
    requests[i] =
      connections[i]->register_addr_nb(local_addrs[i], peer_addrs[i]);
  }
...
{
  return exchange_addrs(
    connections, local_addrs, nullptr, peer_addrs, nullptr, num_conns);
}
                  ...
                  int num_conns)
{
  ...
  for (int i = 0; i < num_conns; ++i)
  {
    ...
    {
      logging::MPIPrintStreamDebug()
        << "MPI exchange with " << conn->get_peer() << "\n";
      conn->sendrecv(local_src_bufs[i],
                     local_sizes[i],
                     local_dst_bufs[i],
                     peer_sizes[i],
                     streams[i]);
    ...
    else if (typeid(*conn_ptr) == typeid(ConnectionIPC))
    {
      p2p::logging::MPIPrintStreamDebug()
        << "IPC exchange with " << conn->get_peer() << "\n";
      conn->put(
        local_src_bufs[i], peer_dst_bufs[i], local_sizes[i], streams[i]);
