...
__global__ void sync_pairwise_kernel(
  int peer, bool notify, bool wait, SyncType sync_type, PairwiseSyncDevice sync)
{
  sync.sync(peer, notify, wait, sync_type);
}

__global__ void
notify_kernel(int peer, SyncType sync_type, PairwiseSyncDevice sync)
{
...
void PairwiseSync::notify(int peer, SyncType sync_type, cudaStream_t stream)
{
  if (peer == MPI_PROC_NULL)
    return;
...
void PairwiseSync::alloc_buffers()
{
  ...
  CounterType* shmem_counter =
    static_cast<CounterType*>(nvshmem_malloc(sizeof(CounterType)));
  ...
  DISTCONV_CHECK_CUDA(cudaMemcpy(
    local_counter, &counter_init, sizeof(CounterType), cudaMemcpyHostToDevice));
  m_local_counter =
    std::shared_ptr<CounterType>(local_counter, [](CounterType* ptr) {
...
__global__ void sync_kernel(int peer,
                            bool notify,
                            bool wait,
                            SyncType sync_type,
                            int idx,
...
__global__ void
notify_kernel(int peer, SyncType sync_type, int idx, SyncArrayDevice sync)
{
...
void SyncArray::notify(int peer,
                       SyncType sync_type,
                       int idx,
                       cudaStream_t stream)
{
...
void SyncArray::alloc_counters()
{
  ...
  CounterType* shmem_counter =
    static_cast<CounterType*>(nvshmem_malloc(sizeof(CounterType) * m_size));
  // util::MPIPrintStreamDebug() << "shmem flag: " << p;
  if (shmem_counter == nullptr)
  {
    util::MPIPrintStreamError() << "Allocation of shmem buffer failed";
...
{
  DISTCONV_CHECK_CUDA(
    cudaMemset(m_shmem_counter.get(), 0, sizeof(CounterType) * m_size));
  std::vector<CounterType> init(m_size, 1);
  DISTCONV_CHECK_CUDA(cudaMemcpy(m_local_counter.get(),
                                 init.data(),
                                 sizeof(CounterType) * m_size,
