...
public:
  ...
  std::ostream& print_as_row(std::ostream& os)
  {
    for (size_t i = 0; i < fwd_time.size(); ++i)
    {
      m_cfg.print_as_row(os)
        << " " << fwd_time[i] << " " << fwd_allreduce_time[i] << " "
        << bwd_time[i] << " " << bwd_allreduce_time[i];
      os << std::endl;
    }
    return os;
  }
  ...
  void print_summary(std::ostream& os)
  {
    std::cout << "Forward mean: " << get_mean(fwd_time)
              ...
              << ", max: " << get_max(bwd_allreduce_time) << std::endl;
  }
};
...
struct BNTester<NSD, cudnn::BackendCUDNN, DataType>
{
                 ...
                 Profile<NSD>& prof)
  {
    ...
    {
      // mean and var
      auto mean_and_var_nvshmem = static_cast<DataType*>(nvshmem_malloc(
        d.mean_and_var.get_max_local_real_shape().size() * sizeof(DataType)));
      ...
      assert0(tensor::View(
        d.var, d.mean_and_var.get_buffer() + d.mean.get_local_real_size()));
      // d_mean and d_var
      auto d_mean_and_var_nvshmem = static_cast<DataType*>(nvshmem_malloc(
        d.d_mean_and_var.get_max_local_real_shape().size() * sizeof(DataType)));
      ...
      assert0(tensor::View(d.d_var,
                           d.d_mean_and_var.get_buffer()
                             + d.d_mean.get_local_real_size()));
      // scale_gradient
      auto d_scale_nvshmem = static_cast<DataType*>(nvshmem_malloc(
        d.d_scale.get_max_local_real_shape().size() * sizeof(DataType)));
      assert_always(d_scale_nvshmem != nullptr);
      assert0(tensor::View(d.d_scale, d_scale_nvshmem));
      // bias_gradient
      auto d_bias_nvshmem = static_cast<DataType*>(nvshmem_malloc(
        d.d_bias.get_max_local_real_shape().size() * sizeof(DataType)));
