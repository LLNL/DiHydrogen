...
public:
  ...
  std::ostream& print_as_row(std::ostream& os)
  {
    for (size_t i = 0; i < fwd_time.size(); ++i)
    {
      m_cfg.print_as_row(os)
        << " " << fwd_time[i] << " " << bwd_time[i] << std::endl;
    }
    return os;
  }
  ...
  void print_summary(std::ostream& os)
  {
    ...
    ss << "Forward mean: " << get_mean(fwd_time)
       << ", median: " << get_median(fwd_time) << ", min: " << get_min(fwd_time)
       << ", max: " << get_max(fwd_time) << std::endl
       << "Backward mean: " << get_mean(bwd_time)
       << ", median: " << get_median(bwd_time) << ", min: " << get_min(bwd_time)
       << ", max: " << get_max(bwd_time) << std::endl;
    os << ss.str();
  }
};
  ...
  Data(const BenchmarkConfig<NSD>& cfg, MPI_Comm comm) : m_cfg(cfg)
  {
    ...
    if (pid == 0)
    {
      ...
      ss << "Input tensor shape: " << input.get_shape()
         << ", distribution: " << input.get_distribution() << std::endl
         << "Output tensor shape: " << output.get_shape()
         << ", distribution: " << output.get_distribution() << std::endl
         << "Derivative of input tensor shape: " << d_input.get_shape()
         << ", distribution: " << d_input.get_distribution() << std::endl
         << "Derivative of output tensor shape: " << d_output.get_shape()
         << ", distribution: " << d_output.get_distribution() << std::endl;
      std::cout << ss.str();
    }
    ...
    util::MPIPrintStreamDebug()
      ...
      << ", d_output tensor: " << d_output << ", d_input tensor: " << d_input;
  }
                  ...
                  Profile<NSD>& prof)
{
  ...
  {
    pooling.backward(
      DataType(1.0), d.output, d.d_output, d.input, DataType(0.0), d.d_input);
  }
  pooling.wait();
  util::MPIRootPrintStreamInfo()
    << "Starting " << cfg.run_count << " times of measurement";
...
struct PoolingTester<NSD, cudnn::BackendCUDNN, DataType>
{
                 ...
                 Profile<NSD>& prof)
  {
    ...
    cudnn::Options be_opts(cfg.overlap_halo_exchange, cfg.deterministic);
    cudnn::BackendCUDNN be(comm, cudnn_h, be_opts);
    if (cfg.nvtx_marking)
    {
      be.enable_nvtx_marking();
    }
    test_forward<NSD, cudnn::BackendCUDNN, DataType>(d, cfg, comm, be, prof);
    test_backward<NSD, cudnn::BackendCUDNN, DataType>(d, cfg, comm, be, prof);
    return 0;
  }
};
#endif
