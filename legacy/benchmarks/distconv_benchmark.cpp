...
public:
  ...
  std::ostream& print_as_row(std::ostream& os)
  {
    ...
    {
      m_cfg.print_as_row(os)
        << " " << conv_fwd_time[i] << " " << conv_bwd_data_time[i] << " "
        << conv_bwd_filter_time[i] << " " << conv_bwd_bias_time[i];
...
public:
  ...
  Data(const BenchmarkConfig<NSD>& cfg, MPI_Comm comm) : m_cfg(cfg)
  {
    ...
    assert_always(
      std::accumulate(cfg.p_s.begin(), cfg.p_s.end(), 1, std::multiplies<int>())
        * cfg.p_c * cfg.p_n
      == np);
    ...
    filter = create_filter_tensor<Tensor>(locale_shape,
                                          filter_dims,
                                          input,
                                          cfg.i_c,
                                          cfg.f_k,
    ...
    if (!cfg.deconv)
    {
      output = create_convolution_output_tensor<Tensor>(
        input, filter, strides, pads, dilations, cfg.num_groups);
      d_output =
        create_convolution_d_output_tensor<Tensor>(output, filter, dilations);
    }
    else
    {
      output = create_deconvolution_output_tensor<Tensor>(
        input, filter, strides, pads, dilations, cfg.num_groups);
      d_output =
        create_deconvolution_d_output_tensor<Tensor>(output, filter, dilations);
    }
  ...
  bool is_empty() const { return output.get_size() == 0; }
  void initialize()
  {
    // Initialization
                             ...
                             Profile<NSD>& prof)
{
  ...
  util::MPIRootPrintStreamInfo()
    << "Executing test_convolution_forward with " << be.get_name();
  ...
  conv.wait();
  util::MPIRootPrintStreamInfo()
    << "Starting " << cfg.run_count << " times of measurement";
  ...
  for (int i = 0; i < cfg.run_count; ++i)
  {
    ...
    for (int j = 0;
         j
         < std::max(2, (*std::max_element(cfg.p_s.begin(), cfg.p_s.end())) - 1);
         ++j)
    {
      conv.forward(DataType(1.0),
                   d.input,
                                   ...
                                   Profile<NSD>& prof)
{
  ...
  util::MPIRootPrintStreamInfo()
    << "Executing test_convolution_backward_data with " << be.get_name();
  ...
  bool call_halo_exch_separately =
    conv.is_overlap_bwd_halo_exchange_enabled() && !cfg.skip_halo_exchange;
  ...
  for (int i = 0; i < cfg.warming_up_count; ++i)
  {
    ...
    conv.backward_data(DataType(1.0),
                       d.filter,
                       d.d_output,
                       DataType(0.0),
                       d.d_input,
                                     ...
                                     Profile<NSD>& prof)
{
  ...
  util::MPIRootPrintStreamInfo()
    << "Executing test_convolution_backward_filter with " << be.get_name();
  ...
  for (int i = 0; i < cfg.warming_up_count; ++i)
  {
    conv.backward_filter(DataType(1.0),
                         d.input,
                         d.d_output,
                         DataType(0.0),
                         d.d_filter,
                                   ...
                                   Profile<NSD>& prof)
{
  ...
  util::MPIRootPrintStreamInfo()
    << "Executing test_convolution_backward_bias with " << be.get_name();
  ...
  for (int i = 0; i < cfg.run_count; ++i)
  {
    ...
    clk.start();
    conv.backward_bias(DataType(1.0),
                       d.d_output,
                       DataType(0.0),
                       d.d_bias,
                       !cfg.skip_weight_allreduce);
                              ...
                              Profile<NSD>& prof)
{
  ...
  util::MPIRootPrintStreamInfo()
    << "Executing test_convolution_backward with " << be.get_name();
  ...
  for (int i = 0; i < cfg.run_count; ++i)
  {
    ...
    if (!cfg.testing)
    {
      for (int j = 0;
           j < std::max(
             2, (*std::max_element(cfg.p_s.begin(), cfg.p_s.end())) - 1);
           ++j)
      {
        if (call_halo_exch_separately)
        {
...
struct ConvolutionTester<NSD, cudnn::BackendCUDNN, DataType>
{
                 ...
                 Profile<NSD>& prof)
  {
    ...
    cudnn::Options be_opts(
      cfg.overlap_halo_exchange, cfg.deterministic, cfg.profiling);
    ...
    if (cfg.conv_fwd_algo == "AUTOTUNE" || cfg.conv_bwd_data_algo == "AUTOTUNE"
