...
/** @file
 *
 * Local tensors that live on GPUs.
 */
...
template <typename T>
class Tensor<T, Device::GPU> : public BaseTensor<T> {
public:
  using value_type = T;
  ...
  Tensor(ShapeTuple shape_,
         ...
         const SyncInfo<Device::GPU>& sync = SyncInfo<Device::GPU>{}) :
  ...
  Tensor(const SyncInfo<Device::GPU>& sync = SyncInfo<Device::GPU>{})
  ...
  void empty() override
  {
    ...
    tensor_memory = StridedMemory<T, Device::GPU>(sync);
  ...
  void resize(ShapeTuple new_shape) override {
    ...
    auto sync = tensor_memory.get_sync_info();
    tensor_memory = StridedMemory<T, Device::GPU>(new_shape, sync);
  ...
  Tensor<T, Device::GPU>* contiguous() override {
    if (is_contiguous()) {
      return view();
    }
  ...
  Tensor<T, Device::GPU>* view() override {
    return view(CoordTuple(TuplePad<CoordTuple>(this->tensor_shape.size(), ALL)));
  }

  Tensor<T, Device::GPU>* view() const override
  {
    return view(CoordTuple(TuplePad<CoordTuple>(this->tensor_shape.size(), ALL)));
  }

  Tensor<T, Device::GPU>* view(CoordTuple coords) override
  {
    return make_view(coords, ViewType::Mutable);
  }

  Tensor<T, Device::GPU>* view(CoordTuple coords) const override
  {
    return make_view(coords, ViewType::Const);
  }

  Tensor<T, Device::GPU>* operator()(CoordTuple coords) override
  {
    return view(coords);
  }
  ...
  Tensor<T, Device::GPU>* const_view() const override {
    return const_view(CoordTuple(TuplePad<CoordTuple>(this->tensor_shape.size(), ALL)));
  }

  Tensor<T, Device::GPU>* const_view(CoordTuple coords) const override
  {
    return make_view(coords, ViewType::Const);
  }

  Tensor<T, Device::GPU>* operator()(CoordTuple coords) const override {
  ...
  SyncInfo<Device::GPU> get_sync_info() const H2_NOEXCEPT
  {
    return tensor_memory.get_sync_info();
  }

  void set_sync_info(const SyncInfo<Device::GPU>& sync)
  {
    if (this->is_view())
    {
...
private:
  ...
  StridedMemory<T, Device::GPU> tensor_memory;

  /** Private constructor for views. */
  Tensor(ViewType view_type_, const StridedMemory<T, Device::GPU>& mem_,
